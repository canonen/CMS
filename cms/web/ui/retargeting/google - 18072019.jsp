...
async function updateSigninStatus(isSignedIn, alreadyLoggedIn) {
	if (isSignedIn) {
		...
		if(alreadyLoggedIn) {
			...
			const { accountList } = await fetch('https://dev.revotas.com/cms/GoogleApiServlet/fetchAccountList?refresh_token=' + refresh_token)
			.then(resp=>resp.json());
			...
			.then(resp=>resp.json());
			if(!response.success) {
					response = await fetch('https://dev.revotas.com/cms/GoogleApiServlet/authGoogle?code=' + code)
					.then(resp=>resp.json());
					addSettings = true;
			}
			refresh_token = response.refresh_token;
...
async function updateSettings()
{
	...
	if(!user_id || !accountIds || !accountNames || !refresh_token)
		return;
	const params = {cust_id,revotas_user,user_id,accountIds,accountNames,refresh_token};
	let paramString = '?';
