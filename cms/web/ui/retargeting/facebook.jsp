...
function connectedFB(userid,loginCeheck)
{
	...
	FB.api('/'+userid+'', function(response) {
		...
		if(redirect_to==='facebook')
			window.location = 'https://dev.revotas.com/cms/ui/retargeting/retargeting_export_type.jsp?cust_id=<%=sCustId%>&revotas_user=<%=revotasUser%>&selected=facebook';
...
function saveAdAccounts()
{
	...
	if(!(acID=="" || acName=="" ||fuser==""))
		...
		//extend the token
		fetch('https://graph.facebook.com/v4.0/oauth/access_token?grant_type=fb_exchange_token&client_id=278434559502747&client_secret=80fd5855769179cf568452958cc28e8e&fb_exchange_token='+refreshToken)
		.then(resp=>resp.json())
		.then(resp=>{
			//send to database	
