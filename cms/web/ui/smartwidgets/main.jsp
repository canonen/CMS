...
<body>
    <div class="main-container" style="display:flex;justify-content: flex-start">
	    ...
	    <span style="margin-bottom: 10px;">
            ...
            <select class="config-select" id="type">
                ...
                <option value="drawerNoOverlay">Drawer (No overlay)</option>
            ...
            <div class="input-group">
                ...
                <div class="input-subgroup">
                    ...
                    <div class="group">
                        <input class="widget-control form-input text-input" id="preview-size" type="text" value="120px"/>
                        <label class="widget-control form-input-label" for="preview-size">Preview Size</label>
                    </div>
                    ...
                    <div class="group">
                        <textarea style="resize:none;" class="widget-control form-input text-input" id="script-code" rows="1" readonly></textarea>
                        <label class="widget-control form-input-label shrink" for="script-code">Script</label>
                    </div>

                    <div class="group" style="margin-top:6px;">
                        <textarea style="resize:none;" class="content-type-control form-input text-input" id="HTML" rows="1" readonly><h1>TEST</h1></textarea>
                        <label class="content-type-control form-input-label shrink" for="HTML">HTML</label>
                    </div>
                </div>
            ...
            <label class="widget-control label" for="overlay-click">Overlay Click</label>
            <select class="widget-control config-select" id="overlay-click">
                <option value="donotclose" selected>Do not close</option>
                <option value="close">Close</option>
            </select>
            
            <label class="widget-control label" for="drawer-start-state">Starting State</label>
            <select class="widget-control config-select" id="drawer-start-state">
            ...
            <label class="widget-control label" for="overlay-lock">Overlay Lock</label>
            <select class="widget-control config-select" id="overlay-lock">
            ...
            <div class="group">
                <input class="widget-control form-input text-input" id="fixed-elements" type="text"/>
                <label class="widget-control form-input-label" for="fixed-elements">Fixed elements</label>
            </div>
            
            <div class="group">
                <input class="widget-control form-input text-input" id="fixed-elements-unaffected" type="text"/>
                <label class="widget-control form-input-label" for="fixed-elements-unaffected">Fixed elements(unaffected)</label>
            </div>
	...
	<%
		...
		finally
		{
            ...
            <div>
                ...
                <button class="smart-widget-button" id="clonePopup">Clone</button>
            </div>
    ...
    <script>
        ...
        var isEnabled = getSmartWidgetParams().enabled ? '1' : '0';
		...
		fetch('http://cms.revotas.com/cms/ui/smartwidgets/save_smartwidget_config.jsp?enabled='+isEnabled+'&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=<%=popup_id%>',{	
			method: 'POST',
			headers: {
			...
			fetch('http://f.revotas.com/frm/smartwidgets/save_smartwidget_config.jsp?enabled='+isEnabled+'&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=<%=popup_id%>',{	
				method: 'POST',
				headers: {
					'Content-Type':'application/json'
				},
                ...
                fetch('http://'+rcp_url+'/rrcp/imc/smartwidgets/save_smartwidget_config.jsp?enabled='+isEnabled+'&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=<%=popup_id%>',{	
                    method: 'POST',
                    headers: {
                        'Content-Type':'application/json'
                    },
        ...
        document.getElementById('clonePopup').addEventListener('click', function() {
            var cloneAnswer = confirm('Are you sure you want to clone this configuration?');
            if(!cloneAnswer)
                return;
            
            var cloneConfig = getSmartWidgetParamsToSend();
            cloneConfig.enabled = false;
            var clonePopupId = [...Array(30)].map(i=>(~~(Math.random()*36)).toString(36)).join('');
            var form_id = document.getElementById('form_id').value;
    		var popup_name = document.getElementById('popup_name').value + '_Clone';
            fetch('http://cms.revotas.com/cms/ui/smartwidgets/save_smartwidget_config.jsp?enabled=0&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=' + clonePopupId,{	
                method: 'POST',
                headers: {
                    'Content-Type':'application/json'
                },
                ...
                fetch('http://f.revotas.com/frm/smartwidgets/save_smartwidget_config.jsp?enabled=0&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=' + clonePopupId,{	
                    method: 'POST',
                    headers: {
                        'Content-Type':'application/json'
                    },
                    ...
                    fetch('http://'+rcp_url+'/rrcp/imc/smartwidgets/save_smartwidget_config.jsp?enabled=0&popup_name='+popup_name+'&form_id='+form_id+'&cust_id=<%=cust_id%>&popup_id=' + clonePopupId,{	
                        method: 'POST',
                        headers: {
                            'Content-Type':'application/json'
                        },
                        ...
                        window.location = 'main.jsp?cust_id=<%=cust_id%>&popup_id=' + clonePopupId;
                    });
            });
