		...
		if (uploadItem.isFormField()) {
			...
			if (UZANTI[1].equals("png") || UZANTI[1].equals("jpg")|| UZANTI[1].equals("jpeg")|| UZANTI[1].equals("gif")) {
				...
				if (SIZE < MAXSIZE) {
					...
					Date now = new Date();
					SimpleDateFormat format = new SimpleDateFormat("dMyyyy-hhmmss");
					String url = "webpush/img/"+Cust_id+"/"+format.format(now) +"_icon"+ "." + UZANTI[1];
					...
					if (!(blobServiceClient.getBlobContainerClient(containerName).exists())) {
						...
						File fNew = new File(AnaPath + File.separator,format.format(now)+"_icon" + "." + UZANTI[1]);
	                    ...
	                    String img_url = DOMAIN_URL + "/" + Cust_id	+ "/" + format.format(now)+"_icon" + "." + UZANTI[1];
