...
//	Filter sFilterMain1 = createRecursiveFilter(sFilterId);
    Filter sFilterMain1 = new Filter(sFilterId);
    FilterRetrieveUtil.retrieve4UI(sFilterMain1);

    sFilterXml = sFilterMain1.toXml();

    init();
	filterIdList.clear();
	sFilterXml = sFilterXml.replaceAll("<filter_params/>", "");
	sFilterXml = sFilterXml.replaceAll("<filter_parts/>", "");
	sFilterXml = sFilterXml.replaceAll("<formula/>", "");
...
// === === ===
		Filter sFilterMain = new Filter(sFilterId);
...
<%!
	...
	Filter finalFilter = new Filter();
	Filter RecursiveParentFilter = new Filter();
	...
	public Filter createRecursiveFilter (String filterId) {
...
//			if(getParentId(filterId).equalsfinalFilter.m_FilterParts ("")){
//				finalFilter.m_FilterParts = finalFilterParts;
//			}

            if (finalFilterParts.size() > 0 || mainFilterId.equals(filterId)) {
                finalFilter.m_FilterParts = finalFilterParts;
            }
	...
	public Filter createOnlyAFilter (String filterId){
		Filter filter = null;
		try {
	...
	public FilterPart getFilterPart(String filterId) {
		...
		try {
			...
			if (rs4.next()) {
				...
				LChildfilter = new Filter(filterId);
	...
	public void init (){
		LfilterParts = new FilterParts();
		finalFilterParts = new FilterParts();
		LfilterPart = new FilterPart();
		finalFilter = new Filter();;
		RecursiveParentFilter = new Filter();
		ArrayList<String> filterIdList = new ArrayList();
		String ListFilterId = null;
		String mainFilterId = null;
		String localFilterId = null;
