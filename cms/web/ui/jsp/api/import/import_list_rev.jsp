...
<%
    ...
    try {
        ...
        String whereCond = "AND b.batch_id = "+sBatchID+" \n";
        if (sSelectedCategoryId == null || sSelectedCategoryId.equals("0")) {
            ...
            while (resultSet.next()) {
                ...
                String cachedCat = categoryCache.get(batchId.toString());
                if (cachedCat == null) {
                    if (categoryStmt == null) {
                        categoryStmt = connection.prepareStatement("SELECT category_id FROM ccps_object_category WHERE cust_id = ? AND object_id = ?");
                    }
                    categoryStmt.setString(1, cust.s_cust_id);
                    categoryStmt.setString(2, batchId.toString());
                    ResultSet categoryRs = categoryStmt.executeQuery();
                    while (categoryRs.next()) {
                        cachedCat = categoryRs.getString(1);
                    }
                    categoryRs.close();
                    categoryCache.put(batchId.toString(), cachedCat);
                }
                if (cachedCat != null) {
                    data.put("category_id", cachedCat);
                }
...
//Bir önceki sorgularda zaten batch list çekildiği için ekstra olarak çekmeye gerek yok. >> EK 26.11.2024
        System.out.println("----------BatchList skipped (already fetched)------------");

        // Reuse previously built 'array' without re-querying the database
        dataArray.put(array);
        ...
        if (sSelectedCategoryId == null || sSelectedCategoryId.equals("0")) {
            ...
            while (resultSet.next()) {
                ...
                data.put("importDate", importDate == null ? "null" : importDate);
                ...
                String cachedCat = categoryCache.get(importId.toString());
                if (cachedCat == null) {
                    if (categoryStmt == null) {
                        categoryStmt = connection.prepareStatement("SELECT category_id FROM ccps_object_category WHERE cust_id = ? AND object_id = ?");
                    }
                    categoryStmt.setString(1, cust.s_cust_id);
                    categoryStmt.setString(2, importId.toString());
                    ResultSet categoryRs = categoryStmt.executeQuery();
                    while (categoryRs.next()) {
                        cachedCat = categoryRs.getString(1);
                    }
                    categoryRs.close();
                    categoryCache.put(importId.toString(), cachedCat);
                }
                if (cachedCat != null) {
                    data.put("category_id", cachedCat);
                }
...
//            out.println(gson.toJson(el));
        ...
        while (resultSet.next()) {
            ...
            data.put("import_date", importDate == null ? "null" : importDate);
            data.put("status_name", statusName);
            data.put("batch_name", batchName);
