...
<%
	...
	if(orgFileName != null) {
		 fileChars = orgFileName.split("\\.");
		fileName = fileChars[0];
		fileExtension = fileChars[1];
	}
...
//		}
		...
		if (isOriginTicket) {
            ...
            String sqlUpdateStatus = "UPDATE shlp_support_ticket SET status_id = ?  WHERE ticket_id = ?";
            ps = connection.prepareStatement(sqlUpdateStatus);
            ps.setInt(1, 10);
            ps.setInt(2, Integer.parseInt(originTicketId));
            ps.executeUpdate();

			String sqlOriginTicketUpdate = "UPDATE shlp_support_ticket SET origin_ticket_id = ?  WHERE ticket_id = ?";
			ps = connection.prepareStatement(sqlOriginTicketUpdate);
			ps.setInt(1, Integer.parseInt(originTicketId));
			ps.setInt(2, ticketId);
			ps.executeUpdate();


			String sqlModifyDateUpdate = "UPDATE shlp_support_ticket SET modify_date = ? WHERE ticket_id = ?";
			ps = connection.prepareStatement(sqlModifyDateUpdate);
			...
			if(completed != null && completed.equals("true")) {
				...
				String sqlCompleteDateUpdate = "UPDATE shlp_support_ticket SET completed_date = ?  WHERE ticket_id = ?";
				ps = connection.prepareStatement(sqlCompleteDateUpdate);
				ps.setTimestamp(1, sqlTimestamp);
				ps.setInt(2, ticketId);
				ps.executeUpdate();
			}
			...
			if(dueDate != null && !dueDate.equals("")){
				...
				String sqlDueDateUpdate = "UPDATE shlp_support_ticket SET due_date = ?  WHERE ticket_id = ?";
				ps = connection.prepareStatement(sqlDueDateUpdate);
			...
			String sqlTicketCount = "SELECT COUNT(*) as count FROM shlp_support_ticket  WHERE origin_ticket_id = ?";
			ps = connection.prepareStatement(sqlTicketCount);
			ps.setInt(1, Integer.parseInt(originTicketId));
			rs = ps.executeQuery();
			if(rs.next()) {
			...
			if (isFirstResponse){
				String sqlFirstResponseDateUpdate = "UPDATE shlp_support_ticket SET first_response_date = ? WHERE ticket_id = ?";
				ps = connection.prepareStatement(sqlFirstResponseDateUpdate);
				ps.setTimestamp(1, sqlTimestamp);
				ps.setInt(2, Integer.parseInt(originTicketId));
				ps.executeUpdate();
			}
