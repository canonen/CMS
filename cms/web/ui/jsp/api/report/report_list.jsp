...
// Connection
    ...
    try {
        ...
        sSQL = "EXEC usp_crpt_camp_list3 @cust_id=" + cust.s_cust_id;
        ...
        if (stmt.execute(sSQL)) {
            ...
            while (rs.next()) {
                ...
                String updateDateFormatted = (updateTimestamp != null) ? updateTimestamp.toLocalDateTime().format(timeFormatter) : "-";
                data.put("updateDate", updateDateFormatted);


                Timestamp startTimestamp = rs.getTimestamp("startDate");
                String startDateFormatted = (startTimestamp != null) ? startTimestamp.toLocalDateTime().format(timeFormatter) : "-";
                data.put("startDate", startDateFormatted);

                data.put("size",formatter.format(rs.getDouble(6)));
                data.put("tReads", formatter.format(rs.getDouble(17)) + " (" + rs.getString(18) + "%)");
                data.put("tReadsPrc", rs.getString(18));
                data.put("bbacks", formatter.format(rs.getDouble(7)) + " (" + rs.getString(13) + "%)");
                data.put("clicks",formatter.format(rs.getDouble(8))  + " (" + rs.getString(14) + "%)");
