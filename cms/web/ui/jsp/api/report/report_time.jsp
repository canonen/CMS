...
// end (part of release 5.9)
        ...
        if ((CampID != null) && (CampID != "")) {
            ...
            //KU 2004-02-20
            rs = stmt.executeQuery("SELECT count(*) FROM crpt_camp_pos WHERE camp_id  =" + CampID);
...
//						graph_val4 	= graph_val4.substring(0, graph_val4Count - 1);
            ...
            String bbacks = "";
            arrayData = new JsonArray();
            rs = stmt.executeQuery("SELECT sent,dist_reads,dist_clicks,unsubs,bbacks FROM crpt_camp_summary WHERE camp_id= " + CampID);
            while (rs.next()) {
                data = new JsonObject();
                sent = rs.getString(1);
                reads = rs.getString(2);
                clicks = rs.getString(3);
                unsubs = rs.getString(4);
                bbacks = rs.getString(5);

                data.put("sent", sent);
                data.put("reads", reads);
                data.put("clicks", clicks);
                data.put("unsubs", unsubs);
                data.put("bounceback", bbacks);

                arrayData.put(data);
            }
            dataValues.put("summary", arrayData);
