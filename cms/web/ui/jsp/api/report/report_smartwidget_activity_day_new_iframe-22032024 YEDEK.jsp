...
<%
	...
	try{
		...
		if(d_startdate!=null){
			...
			sSql_day = "select CONVERT(VARCHAR(10), activity_date, 120) DAY, count(*), popup_id, form_id, type_name, activity, impression, total_revenue from ccps_smart_widget_activity_day with(nolock) WHERE "
					+"cust_id="+sCustId+" AND activity_date >='"
					+d_startdate+"' AND activity_date<='"
					+d_enddate+"' AND popup_id ='"+popupId+"' GROUP BY CONVERT(VARCHAR(10), activity_date, 120), popup_id, form_id, type_name, activity, impression, total_revenue ORDER BY 1";

		}else{

			sSql_day = "select CONVERT(VARCHAR(10), activity_date, 120) DAY, count(*), popup_id, form_id, type_name, activity, impression, total_revenue from ccps_smart_widget_activity_day with(nolock) WHERE "
					+"cust_id="+sCustId+" AND popup_id ='"+popupId+"' AND activity_date >= DATEADD(day, -30, getdate()) GROUP BY CONVERT(VARCHAR(10), activity_date, 120), popup_id, form_id, type_name, activity, impression, total_revenue ORDER BY 1";
		...
		String total_revenue = rs.getString(8);

		activityObj.put("day",day );
		activityObj.put("count", count);
		...
		activityObj.put("total_revenue", total_revenue);
