...
<%
	...
	String revenue = "";
	String impression = "";
	String config_param_type = "";
	String config_param_enabled = "";
	...
	try {
		...
		String sSql = "SELECT c.popup_name, c.popup_id, c.modify_date, c.create_date, c.order_number, c.status, p.config_param_type, p.config_param_enabled, " +
						...
						"LEFT JOIN c_smart_widget_config_parsed AS p ON c.id = p.id " +
						"WHERE c.status <> 90 AND c.cust_id = " + custId + " " +
						"GROUP BY c.popup_name, c.popup_id, c.modify_date, c.create_date, c.order_number, c.status, p.config_param_type, p.config_param_enabled ORDER BY c.order_number ";
		...
		while (rs.next()) {
			...
			config_param_type = rs.getString("config_param_type");
			config_param_enabled = rs.getString("config_param_enabled");
			...
			data.put("config_param_type", config_param_type);
			data.put("config_param_enabled", config_param_enabled);
