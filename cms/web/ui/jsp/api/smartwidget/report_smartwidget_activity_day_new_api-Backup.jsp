...
<%
    ...
    try {
        ...
        while (rs.next()) {
            ...
            //JsonObject jsonConfigParam = new JsonObject(config_param);
            String create_date = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT).format(rs.getTimestamp(3));
            String modify_date = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT).format(rs.getTimestamp(4));
            String status = rs.getString(5);
            String click = rs.getString(6);
            String submit = rs.getString(7);
            String view = rs.getString(8);
            String revenue = rs.getString(9);
            String typeName = rs.getString(10);

            data.put("popup_id", popup_id);
            data.put("popup_name", popup_name);
            //data.put("config_param", jsonConfigParam);
            ...
            data.put("view", view == null ? "0" : formatInteger(Long.parseLong(view)));
        ...
        String totalViewSqlQuery = "select  sum(impression) as totalview  from ccps_smart_widget_activity_day with (nolock) WHERE cust_id=" + sCustId + "  AND activity_date >= DATEADD(day," + "-" + dayDifference + ", '" + currentValueEndDate + "')";
        String totalViewSqlQuery2= "select sum(impression) as totalview from ccps_smart_widget_activity_day with (nolock)  WHERE cust_id ="+sCustId+" AND activity_date between '"+currentValueStartDate+"' and '"+currentValueEndDate+"'";
        ...
        resultSet.close();
        String totalRevenueSqlQuery = "select   sum(revenue) as revenue    from ccps_smart_widget_activity_day with (nolock) WHERE cust_id=" + sCustId + "  AND activity_date >= DATEADD(day," + "-" + dayDifference + ", '" + currentValueEndDate + "')";
        String totalRevenueSqlQuery2= "select sum(revenue) as revenue from ccps_smart_widget_activity_day with (nolock)  WHERE cust_id ="+sCustId+" AND activity_date between '"+currentValueStartDate+"' and '"+currentValueEndDate+"'";
        ...
        resultSet.close();
        String totalClickSqlQuery = "select  sum(activity) as click  from ccps_smart_widget_activity_day with (nolock)  WHERE cust_id=" + sCustId + "  AND activity_date >= DATEADD(day," + "-" + dayDifference + ", '" + currentValueEndDate + "')  and type_name = 1";
        String totalClickSqlQuery2= "select sum(activity) as click from ccps_smart_widget_activity_day with (nolock)  WHERE cust_id ="+sCustId+" AND activity_date between '"+currentValueStartDate+"' and '"+currentValueEndDate+"' and type_name=1";
        ...
        resultSet.close();
        String totalSubmitSqlQuery = "select  sum(activity) as submit  from ccps_smart_widget_activity_day with (nolock) WHERE cust_id=" + sCustId + "  AND activity_date >= DATEADD(day," + "-" + dayDifference + ",'" + currentValueEndDate + "') and type_name = 2 ";
        String totalSubmitSqlQuery2= "select sum(activity) as submit from ccps_smart_widget_activity_day with (nolock)  WHERE cust_id ="+sCustId+" AND activity_date between '"+currentValueStartDate+"' and '"+currentValueEndDate+"' and type_name=2";
