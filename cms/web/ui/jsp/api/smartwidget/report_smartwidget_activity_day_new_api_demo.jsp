...
<%
    ...
    try {
        ...
        stmt.setFetchSize(1000); // SQL Server 2012 için optimize boyut

        // MS SQL Server 2012 uyumlu - parametreli sorgular
        String ultraQuery =
                "SET NOCOUNT ON; " +
                        "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; " +

                        // Basit approach - direk string kullanıyoruz, casting yapmıyoruz
                        "WITH FastData AS ( " +
                        "    SELECT " +
                        "        1 AS query_type, " +
                        ...
                        "    LEFT JOIN ccps_smart_widget_activity_day a WITH (NOLOCK) " +
                        "        ON c.popup_id = a.popup_id " +
                        "        AND a.cust_id = '" + sCustId + "' " +
                        "        AND a.activity_date BETWEEN '" + currentValueStartDate + " 00:00:00' AND '" + currentValueEndDate + " 23:59:59' " +
                        "    WHERE c.cust_id = '" + sCustId + "' AND c.status <> 90 " +
                        "    GROUP BY c.popup_id, c.popup_name, c.create_date, c.modify_date, c.status " +
                        "), " +
                        "TotalData AS ( " +
                        "    SELECT " +
                        "        3 AS query_type, " +
                        ...
                        "ORDER BY query_type, is_current DESC, str_date, type_name";
        ...
        try {
            ...
            logger.error("Database cleanup error", e);
        }
    }
