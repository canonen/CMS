...
<!--- Step 1 Header----->
<table width="600" class="listTable" cellspacing="0" cellpadding="0">
	<tr>
		<th class="sectionheader">&nbsp;<b class="sectionheader">Step 1:</b> User Access Levels for <%= (sUserId==null)?"New User":u.s_user_name %></th>
	</tr>
	<tbody class="EditBlock" id="block1_Step1">
	<tr>
		<td class="" valign="top" align="center" >
			<table class="listTable" border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
		...
		try
		{
			...
			while (rs.next())
			{
				...
				%>
				<tr>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="left" valign="middle">
						<%=sTypeName%>
						<INPUT type="hidden" name=<%=sTypeId%> value=0>
					</td>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="center" valign="middle" nowrap><INPUT type="checkbox" name=<%=sTypeId%> value=<%=AccessRight.READ%> <%=((AccessRight.READ & iMask) == AccessRight.READ)?"checked":""%>></td>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="center" valign="middle" nowrap><INPUT type="checkbox" name=<%=sTypeId%> value=<%=AccessRight.WRITE%> <%=((AccessRight.WRITE & iMask) == AccessRight.WRITE)?"checked":""%>></td>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="center" valign="middle" nowrap><INPUT type="checkbox" name=<%=sTypeId%> value=<%=AccessRight.EXECUTE%> <%=((AccessRight.EXECUTE & iMask) == AccessRight.EXECUTE)?"checked":""%>></td>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="center" valign="middle" nowrap><INPUT type="checkbox" name=<%=sTypeId%> value=<%=AccessRight.DELETE%> <%=((AccessRight.DELETE & iMask) == AccessRight.DELETE)?"checked":""%>></td>
					<td <%if(k % 2 == 0) {%> class="list_row";<%}else{%>class="list_row_other"<%}%> align="center" valign="middle" nowrap><INPUT type="checkbox" name=<%=sTypeId%> value=<%=AccessRight.APPROVE%> <%=((AccessRight.APPROVE & iMask) == AccessRight.APPROVE)?"checked":""%>></td>
				</tr>
				<%
				k++;
			}
