...
// === === ===
    ...
    try {
        ...
        try {
            ...
            //MSSQL'in  IN(?,?,?...) sınırı 2100 olduğundan, bunu aşan kayıtlarda patlamaması için parca parca eklendi.
            int BATCH_SIZE = 1000;

            for (int i = 0; i < filterList.size(); i += BATCH_SIZE){
                List<String> batch =
                        filterList.subList(i, Math.min(i + BATCH_SIZE, filterList.size()));

                if (batch.isEmpty()) break;

                String inClause = String.join(",", batch);

                String categorySqlQuery =
                        "SELECT category_id FROM ccps_object_category WITH(NOLOCK) " +
                                "WHERE cust_id = ? AND object_id IN (" + inClause + ")";
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    try {
                        ...
                        eFilter = com.britemoon.cps.XmlUtil.getRootElement(sFilterXml);
