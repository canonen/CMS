...
else
{
	...
	long responseFilterId = filter.s_filter_id;
		
	JSONObject result = new JSONObject();
	
	result.put("responseFilterId", ++responseFilterId);
	...
	try {
		...
		String query = "SELECT child_filter_id FROM ctgt_filter_part WITH (NOLOCK) WHERE parent_filter_id = " + filter.s_filter_id;
		...
		while (rs.next()) {
			...
			String childFilterId = rs.getString("child_filter_id");
			
			result.put("childFilterId_" + (++i), childFilterId);
		}
		rs.close();
		out.println(result);
	} catch (Exception e) {
