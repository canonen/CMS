...
//			sw.write("</OPTION>");
	...
	while (rs.next()) {
		...
		while(rs_2.next()) {
			...
			dataObjectJson.put("LinkId2", id2);
			++kClick;
			isChecked = "";
			dataObjectJson.put("isChecked","");
			if (kClick == 0) {
				if (isClick) {
					...
					kClick0 = id2;
						...
						if(id2.equals(sSelectedLinkId)){
							...
							dataObjectJson.put("LinkName", sName);
						}
						else {
							sName = new String(rs.getBytes(2),"ISO-8859-1");
	...
	while(elements.hasMoreElements()){
		...
		dataCust.put("preview_name", ca.s_display_name);
	...
	arrayJson.put(array3);
	arrayJson.put(array4);
	arrayJson.put(array2);
	arrayJson.put(array1);
	arrayJson.put(array);
	arrayJson.put(array6);
	arrayJson.put(arrayRetrieve);
	arrayJson.put(arrayPreview);
