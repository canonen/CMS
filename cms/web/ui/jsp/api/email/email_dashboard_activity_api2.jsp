...
//    String lastDate = request.getParameter("lastDate");
    ...
    try {
        ...
        String getClickAndBBackCount = "SELECT" +
                "  SUM(CASE WHEN type_id = 1 THEN rjtk_count ELSE 0 END) AS type_1_sum," +
                "  SUM(CASE WHEN type_id = 2 THEN rjtk_count ELSE 0 END) AS type_2_sum" +
                "  FROM" +
                "  ccps_rjtk_link_activity with(nolock)" +
                " WHERE click_time >='" + currentValueStartDate + "' AND click_time<= '" + currentValueEndDate + "'  AND cust_id = " + sCustId +
                " AND type_id IN (1, 2);";

        resultSet = statement.executeQuery(getClickAndBBackCount);
