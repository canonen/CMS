...
<%
	...
	if (myPart != null && myPart instanceof FilePart) {
		inMjml = ((FilePart) myPart).getInputStream();

		while (inMjml.read(buf) != -1) {
			mjmlTemplate += new String(buf, "UTF-8");
			//Clear out buf
			buf = new byte[1024 * 5];
		}

		if (mjmlTemplate != null) {
			mjmlTemplate = mjmlTemplate.trim();
		}
		inMjml.close();
	}
	//Image File
...
if (templates[0].length() == 0 && mjmlTemplate.length() == 0) errmsg += "<li>Bad file for the HTML Master Template File\n";
if (templates[1].length() == 0 && mjmlTemplate.length() == 0) errmsg += "<li>Bad file for the Text Master Template File\n";
...
TemplateBean tbean = new TemplateBean(templateID, Integer.parseInt(sCustID), templateName, category, fileName, templates, mjmlTemplate);
if (sChildList != null) {
...
<% if (!errmsg.equals("ok") && templates[0].length() > 0) { %>
<%-- Bad template file --%>
<table cellspacing="0" cellpadding="0" width="650" border="0">
	<tr>
		<td class="EmptyTab" valign="center" nowrap align="middle" width="100%"><img height="2" src="../images/blank.gif" width="1"></td>
	...
	<tr>
		<td class=fillTab>
			<table class=main cellspacing=1 cellpadding=2 width="100%">
				...
				<tr>
					<td width="100">Mjml: </td>
					<td><textarea rows="15" cols="80" wrap="off" style="width:100%;"><%= tbean.getTemplate("mjml") %></textarea></td>
				</tr>
				<tr>
