...
<%
	...
	String succesStr = request.getParameter("success");
	if (succesStr != null && succesStr.equals("true")) {
		UploadServlet servlet = new UploadServlet();

		servlet.setCustId(request.getParameter("custId"));
...
class UploadServlet extends HttpServlet {
            ...
            try {
                ...
                for (FileItem item : multipartItems) {
                    if (!item.isFormField()) {
                        ...
                        String fileName = replaceDotsExceptLast(originalFileName);
    ...
    private String replaceDotsExceptLast(String fileName) {
        int lastDotIndex = fileName.lastIndexOf('.');
        if (lastDotIndex == -1) {
            return fileName;
        }

        String nameWithoutExtension = fileName.substring(0, lastDotIndex).replace('.', '_');
