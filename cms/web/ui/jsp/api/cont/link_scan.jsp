...
<%
  ...
  logger.info("debug: saved links 3");
  ...
  JsonObject linkScanObj = new JsonObject();

  linkScanObj.put("cont_id",contID);
  linkScanObj.put("use_anchor_name", useAnchorName);
  linkScanObj.put("use_link_renaming",useLinkRenaming);
  linkScanObj.put("replace_scanned_links:  ",sReplaceScannedLinks);
  linkScanObj.put("loadId",sLoadId);
  //linkScanObj.put("Message:","cont load saved successfully");
  responseJson.put("linkSettings",linkScanObj);

  if(cont.retrieve() < 1) {
    logger.info("debug: saved links 4");
    throw new Exception("Invalid content. Content does not exist.");
  }
  logger.info("debug: saved links 5");

  ContBody cont_body = new ContBody(contID);
  logger.info("debug: saved links 6");
  // === === ===
...
//  String contName = cont.s_cont_name;
  ...
  JSONArray htmlLinkArray = new JSONArray();
//  htmlLinkArray.put(linkScanObj);
  ...
  try
  {
    ...
    long contIdLong = Long.parseLong(contID);
    long custIdLong = Long.parseLong(custId);

// 1. cont_id veritabanında var mı kontrolü
    String selectSql = "SELECT COUNT(*) AS count_cont FROM cjtk_link WHERE cont_id = ?";
    PreparedStatement checkStmt = conn.prepareStatement(selectSql);
    checkStmt.setLong(1, contIdLong);
    ResultSet result = checkStmt.executeQuery();

    boolean gotContInTable = false;
    if (result.next()) {
      gotContInTable = result.getInt("count_cont") > 0;
    }
    result.close();
    checkStmt.close();

// 2. INSERT veya UPDATE sorgusunu hazırla
    String sql;
    PreparedStatement pstmt;

    if (gotContInTable) {
      sql = "UPDATE cjtk_link SET use_link_renaming = ?, replace_scanned_links = ?, use_anchor_name = ? WHERE cont_id = ?";
      pstmt = conn.prepareStatement(sql);

      pstmt.setBoolean(1, useLinkRenaming);
      pstmt.setBoolean(2, replaceScannedLinks);
      pstmt.setBoolean(3, useAnchorName);
      pstmt.setLong(4, contIdLong);

    } else {
      // link_id al
      CallableStatement callStmt = conn.prepareCall("{call usp_ccps_next_id_get(?, ?, ?)}");
      callStmt.setLong(1, custIdLong);
      callStmt.setInt(2, 130); // id_type_id sabit
      callStmt.registerOutParameter(3, Types.INTEGER);
      callStmt.execute();
      int linkId = callStmt.getInt(3);
      callStmt.close();

      sql = "INSERT INTO cjtk_link (link_name, link_id, cust_id, use_link_renaming, replace_scanned_links, use_anchor_name, cont_id) " +
              "VALUES (?, ?, ?, ?, ?, ?, ?)";

      pstmt = conn.prepareStatement(sql);
      pstmt.setString(1, "read_link");
      pstmt.setInt(2, linkId);
      pstmt.setLong(3, custIdLong);
      pstmt.setBoolean(4, useLinkRenaming);
      pstmt.setBoolean(5, replaceScannedLinks);
      pstmt.setBoolean(6, useAnchorName);
      pstmt.setLong(7, contIdLong);
    }
...
// 3. Sorguyu çalıştır
    ...
    while (rs.next()) {
      String href = rs.getString("href")==null?"null":rs.getString("href");
      String linkName = new String(rs.getBytes("link_name"), "UTF-8");
      hCurLinks.put(href, linkName);
    }
    rs.close();
    ...
    logger.info("debug: saved links 1");
    ...
    logger.info("end debug: saved links 2");
    // end debug
    ...
    for (int i=0;i<vLinks.size();++i)
    {
      ...
      JSONObject htmlLinkJson = new JSONObject();
      htmlLinkJson.put("tabindex", iTabCount);
      htmlLinkJson.put("linkCount", linkCount);
      htmlLinkJson.put("notImage", notImage);
      htmlLinkJson.put("oneLink", oneLink);
      htmlLinkJson.put("hCurLinks", hCurLinks);
      htmlLinkArray.put(linkCount, htmlLinkJson);
      ...
      htmlLinkJson.put("oneLink", oneLink);
      htmlLinkJson.put("hCurLinks", hCurLinks);
      htmlLinkArray.put(0, htmlLinkJson);
...
// System.out.println("sLogicBlockId = " + sLogicBlockId);
      ...
      while (rs.next())
      {
        ...
        if (!oldLogicID.equals(tmpLogicID)) {
          htmlLinkJson.put("tmpLogicName", tmpLogicName);

          oldLogicID = tmpLogicID;
        }
        htmlLinkJson.put("tmpContName", tmpContName);

        htmlLinkArray.put(1, "htmlLinkJson");
        vLinks  = scanForHtmlAnchors(tmpHtmlPart+"\n"+tmpAolPart+"\n", hCurLinks, hExactLinks, hPartialLinks, useAnchorName, useLinkRenaming, replaceScannedLinks);
        vLinks2 = scanForTextLinks(tmpTextPart+"\n", hCurLinks, hExactLinks, hPartialLinks, useLinkRenaming, replaceScannedLinks);
        vLinks3 = scanForHtmlImgs(tmpHtmlPart+"\n"+tmpAolPart+"\n");
        vLinks.addAll(vLinks2);
        ...
        for (int i=0;i<vLinks.size();++i)
        {
          ...
          htmlLinkJson.put("oneLink", oneLink);
          htmlLinkJson.put("hCurLinks", hCurLinks);
          htmlLinkArray.put(iTabCount, htmlLinkJson);
  ...
  responseJson.put("htmlLinks",htmlLinkArray);
