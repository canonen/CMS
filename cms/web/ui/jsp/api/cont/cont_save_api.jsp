...
<%
	...
	String sUseAnchorName = BriteRequest.getParameter(request,"use_anchor_name");//link_scan 
	String sUseLinkRenaming = BriteRequest.getParameter(request,"use_link_renaming");//link_scan
	String sReplaceScannedLinks = BriteRequest.getParameter(request,"replace_scanned_links");//link_scan
	// === === ===
	...
	while((configParam = in.readLine() !=null))
	{
		data= new JsonObject(configParam);
	}
	...
	String sTmp=data.getString("ContentHTML");
	...
	sTmp=data.getString("ContentText");
	...
	sTmp=data.getString("ContentAOL")
	...
	String trackUrl = data.getString("TrackURLs";)
	cont.m_Links = getLinks(request, cont.s_cont_id, cont.s_cust_id,trackUrl);
	...
	else if (iActionSave == SAVE_LINKS) sRedirectUrl = "link_scan_save.jsp?cont_id=" + cont.s_cont_id + "&use_anchor_name=" + sUseAnchorName + "&use_link_renaming=" + sUseLinkRenaming + "&replace_scanned_links=" + sReplaceScannedLinks;
	...
	newCont.put("contId",cont.s_cont_id);
	newCont("category_id",sSelectedCategoryId);
	if(iActionSave==SAVE_AS_NEW || iActionSave == SAVE_TO_DEST)
	{
		newCont.put("saveType","cloned")
	}
	else
	{
		newCont.put("saveType","saved")
	}
	array.put(newCont);
...
private static Links getLinks(HttpServletRequest request, String sContId, String sCustId,String strTrackValuesStr)
{
	Links links = null;
	System.out.print("strTrackValuesStr",strTrackValuesStr);
	String[] strTrackValues = strTrackValuesStr.split("");

    System.out.print("strTrackValuesStr",strTrackValues);
	...
	for (int i=0; i<strTrackValues.length; i++)
	{
		...
		System.out.print("item",strTrackValues[i]);
