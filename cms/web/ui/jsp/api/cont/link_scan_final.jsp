...
<%
	...
	String sLoadId = request.getParameter("loadId"); // sLoadId parametre olarak burada aliniyor tablodaki loadId ye Karsilik geliyor
	...
	try
	{
		...
		while (rs.next()){
			data.put("link_name",rs.getString("link_name"));
			data.put("link_definition",rs.getString("link_definition"));
		...
		if (validateContentLoadImages) {
			...
			sSql = "SELECT DISTINCT f.file_url"
				...
				+ " AND f.load_id = "+sLoadId;// burada bir id degeri gelmesi lazim yada postmanda bir deger vermeliyiz!
			
			rs = stmt.executeQuery(sSql);
			if (rs.next()) {
				rootDir =data.put("file_url",rs.getString("file_url")); 
                System.out.println("oguzhan test"+rootDir);
			}
			rs.close();
			idx = rootDir.lastIndexOf('/'); // burada tam olarak yapilan isi anlamadim
			if (idx > 0) {
			...
			sSql =
				...
				" WHERE load_id = " + sLoadId+ // burada bir id degeri gelmesi lazim yada postmanda bir deger vermeliyiz!
				"   AND type_id = " + FileType.IMAGE;			
			rs = stmt.executeQuery(sSql);
			logger.info("rootDir = " + rootDir);
			while (rs.next()) {
				fileUrl = data.put("file_url",rs.getString("file_url"));
		...
		vLinks = scanForHtmlAnchors(tmpHtmlPart,tmpAolPart, hCurLinks, hExactLinks, hPartialLinks, useAnchorName, useLinkRenaming, replaceScannedLinks);
		vLinks2 = scanForTextLinks(tmpTextPart, hCurLinks, hExactLinks, hPartialLinks, useLinkRenaming, replaceScannedLinks);
		vLinks3 = scanForHtmlImgs(tmpHtmlPart,tmpAolPart);
