...
<%
	...
	try
	{
		...
		for (int i=0;i<vPers.size();++i){
        	...
        	aster = "";
            object.put("item5",aster+sArray[1]);
            object.put("id",sArray[0]);
            array.put(object);
        }
		...
		for (Enumeration eLogicBlockIds = vLogicBlockIds.elements() ; eLogicBlockIds.hasMoreElements() ;)
		{
			...
			while (rs.next())
			{
                ...
                obj.put("tmpLogicName",tmpLogicName);
				obj.put("oldLogicId",tmpLogicID);
				obj.put("tmpContName",tmpContName);
				obj.put("tmpFilterName",tmpFilterName);
				obj.put("tmpFilterId",tmpFilterID);


				vPers = scanForPers(htmlPart+textPart+aolPart,hPers);
				ePers = vPers.elements();
                obj.put("inputCount",vPers.size());
