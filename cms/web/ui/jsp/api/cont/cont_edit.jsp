	...
	try {
		...
		if (contID != null) {
			...
			if (rs.next()) {
				...
				dataObject.put("contMJML", contMJML);
				...
				dataObject.put("mjmlFlag", mjmlFlag);
			...
			sSql =
					" SELECT link_id, link_name, cust_id, origin_link_id, cont_id, href, camp_id, entity_id, use_link_renaming, replace_scanned_links, use_anchor_name " +
							" From cjtk_link " +
			...
			while (rs.next()) {
				...
				String use_link_renaming = (rs.getString("use_link_renaming"));
				String replace_scanned_links = (rs.getString("replace_scanned_links"));
				String use_anchor_name = (rs.getString("use_anchor_name"));

				dataObject.put("linkName", linkName);
				dataObject.put("href", href);
				dataObject.put("use_link_renaming", use_link_renaming);
				dataObject.put("replace_scanned_links", replace_scanned_links);
				dataObject.put("use_anchor_name", use_anchor_name);
				arrayObject.put(dataObject);
...
//			CategortiesControl.toHtmlOptions(cust.s_cust_id, ObjectType.CONTENT, contID, sSelectedCategoryId);
	...
	contSaveData.put("mjmlUnsubContent", mjmlUnsubContent);
...
//				}
	...
	if (isPrint) {
		...
		contSaveData.put("contText", contText);
		contSaveData.put("contAOL", contAOL);
		contSaveData.put("contHTML",contHTML);
		contSaveData.put("contMJML", contMJML);
