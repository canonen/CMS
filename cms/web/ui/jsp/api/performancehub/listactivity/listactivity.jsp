...
<%
  ...
  String firstDate = request.getParameter("firstDate") + " 00:00:00";
  String lastDate = request.getParameter("lastDate") + " 23:59:59";
  ...
  try {
    ...
    String sSql_Rate = "";
sSql_Rate  = "SELECT ";
sSql_Rate += " distinctReadPrc = ";
sSql_Rate += "  CASE WHEN SUM(r.sent - r.bbacks) = 0 THEN 0 ";
sSql_Rate += "       ELSE CONVERT(decimal(5,2), (SUM(r.dist_reads) * 100.0) / SUM(r.sent - r.bbacks)) ";
sSql_Rate += "  END,";
sSql_Rate += " distinctClickPrc = ";
sSql_Rate += "  CASE WHEN SUM(r.sent - r.bbacks) = 0 THEN 0 ";
sSql_Rate += "       ELSE CONVERT(decimal(5,2), (SUM(r.dist_clicks) * 100.0) / SUM(r.sent - r.bbacks)) ";
sSql_Rate += "  END,";
sSql_Rate += " BBackPrc = ";
sSql_Rate += "  CASE WHEN SUM(r.sent) = 0 THEN 0 ";
sSql_Rate += "       ELSE CONVERT(decimal(5,2), (SUM(r.bbacks) * 100.0) / SUM(r.sent)) ";
sSql_Rate += "  END";
sSql_Rate += " FROM ccps_rrpt_camp_summary_and_rque_campaign r WITH(NOLOCK)";
sSql_Rate += " WHERE r.start_date >= '" + firstDate + "'";
sSql_Rate += "   AND r.start_date <= '" + lastDate + "'";
sSql_Rate += "   AND r.cust_id = " + sCustId;
  ...
  listActivityDataArray = new JsonArray();
  } finally {
	try {
