...
<%
  ...
  JsonArray rptEcommerceArrayData = new JsonArray();
  try{
    cp = ConnectionPool.getInstance();
    conn = cp.getConnection(this);
    stmt = conn.createStatement();

    Locale american = Locale.US;
    ...
    rptEcommerceArrayData.put(arrayData);
    rs.close();
    ...
    Double sCamp_Purchasers		=null;
    Double sCamp_Purchases		=0.0;
    ...
    while(rs.next()){
      ...
      sCamp_Purchasers		=(rs.getObject(2)==null) ? 0.0 : rs.getDouble(2);
      sCamp_Purchases			= (rs.getObject(3)==null) ? 0.0 : rs.getDouble(3);

      //FH
      sCamp_Sales 			= rs.getBigDecimal(4) == null ? BigDecimal.ZERO : rs.getBigDecimal(4);
      ...
      zCamp_Sales				= formatter2.format(sCamp_Sales);

      sTotal_Sales			= sTotal_Sales.add( sCamp_Sales);

      sCamp_ID	 			= rs.getString(5);
      zCamp_Start_Date	 	= dateFormatter(rs.getTimestamp(6));
      sClicks	 				= rs.getDouble(7);
      sType_ID	 			= rs.getString(8);
      sDaily_Flag	 			= rs.getString(9);
      sCamp_Code	 			= rs.getString(10);


      double intPurchases = sCamp_Purchases;
      int intClicks = (int) sClicks;

      nConversion=(100.0 * intPurchases) /  intClicks;
      nConversion_Formated= String.format("%.2f", nConversion);
      nConversion_Formated = "%" + nConversion_Formated;
      data.put("sCamp_Name",sCamp_Name);
      data.put("sCamp_Purchasers",formatter.format(sCamp_Purchasers));
      data.put("sCamp_Purchases",formatter.format(sCamp_Purchases));
      ...
      data.put("sClicks",formatter.format(sClicks));
    ...
    sAverage_Sales_Formated = formatter3.format(sAverage_Sales);
    ...
    if (sTotal_Clicks_Int == 0) {
      ...
      sConversion_Rate = (100.0 * sTotal_Purchasers_Int) / sTotal_Clicks_Int;
    }
    sConversion_Formated= String.format("%.2f", sConversion_Rate);
    sConversion_Formated = "%" + sConversion_Formated;
    zTotal_Sales 	= (formatter2.format(sTotal_Sales)==null)?"0,0TL":formatter2.format(sTotal_Sales);
  ...
  if(rptEcommerceArrayData.length()==0){
    response.setStatus(204);
  }else{
    out.print(rptEcommerceArrayData.toString());
  }
