...
<%
	...
	if (DELIMITER != null)
	{
		if (DELIMITER.equals ("TAB") || DELIMITER.equals ("Tab") || DELIMITER.equals ("tab"))
			DELIMITER = "\t";
		if (DELIMITER.equals (","))
			DELIMITER = ",";
		if (DELIMITER.equals (";"))
	...
	try
	{
		...
		try
		{
			...
			data.put("file_url", exp.s_file_url);
		}
		catch (Exception e)
		{
		...
		System.out.println(sCategories.length);
		if (sCategories != null && sCategories.length > 0  && sCategories[0] != null && !"".equals(sCategories[0].trim())){
			CategortiesControl.saveCategories(cust.s_cust_id, ObjectType.EXPORT, fileID, request);
		}


		data.put("Export Processing", "The export has been queued.");
