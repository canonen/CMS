...
<section class="content-header" style="margin-left:20px;margin-right:20px;padding:0px;margin-top:10px; height:100%;" >
			...
			<%	if (!"locked".equals(status)) { %>
				 <a class="btn btn-primary" href="pagesave.jsp?rename=true"><%=(isHyatt.equals("1")?"Rename":"Rename/Change Send Type")%></a> 

				<% if ("1".equals(isHyatt)) { %>
				 <a class="btn btn-primary" href="commit.jsp?templateID=<%= templateID %><%= contentID %>"><%=("committed".equals(status))?"Re-Commit":"Commit"%></a> 
			<%	 	} 
				} %>
			...
			else
			{
				 %> <div class="col-md-12"> <%
					...
					{
						%>
						 <a class="btn btn-default " href="pageedit.jsp?previewType=html&isEdit=false&templateID=<%= templateID %><%= contentID %>">HTML</a> 
						 <a class="btn btn-warning" href="#">Text</a> 
						<%	
					}
					if (!"locked".equals(status)) { %>
					 <a class="btn btn-primary" href="pageedit.jsp?isEdit=true&templateID=<%= templateID %><%= contentID %>">Edit Template</a> 
					<% } %>
