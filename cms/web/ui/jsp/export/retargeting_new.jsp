    ...
    <div class="modal-content">
     ...
     <div class="form-group row">
      ...
      <div class="modal-footer">
        <button style="width:50px !important" type="button" class="savebutton" onclick="<%=userType.equals("google") ? "new_audience_google()" : "new_audience()" %>">Create</button>
	...
	<tbody class=EditBlock id=retargeting_Step1>
			...
			<tr>
			   <td width="150">Retargeting Group</td>
				<td>
				 <select id="retargeting" name="retargeting" onchange="display_audience()" disabled>
...
async function change_account_google() {
	...
	const { refresh_token } = await fetch('http://cms.revotas.com/cms/ui/retargeting/get_google_refresh_token.jsp?user_id=<%=userId%>')
								.then(resp=>resp.json());
	const audienceList = await fetch('http://rcp3.revotas.com/rrcp/GoogleApiServlet/fetchAudienceListEncoded?refresh_token=' + refresh_token + '&account_id=' + accountId)
...
var acs=accountID.split(":");
 ...
 accountID=acs[0].trim();
		...
		var url = "http://rcp3.revotas.com/rrcp/ui/retargeting/getAudience.jsp"; 
...
function getTimePeriod(obj)
  ...
  var time_html="<td width=\"150\">Period Type</td>"+
    ...
    "</td>";
	  ...
	  async function new_audience_google() {
		let option = '';
		const accountId = document.getElementById('select_adaccounts').value.split(':')[0];
		const name = document.getElementById('aud_name').value
		const description = document.getElementById('aud_description').value;
		if(!name) {
		...
		const { refresh_token } = await fetch('http://cms.revotas.com/cms/ui/retargeting/get_google_refresh_token.jsp?user_id=<%=userId%>')
									.then(resp=>resp.json());
		const response = await fetch('http://rcp3.revotas.com/rrcp/GoogleApiServlet/createAudienceList?refresh_token=' + refresh_token + '&account_id=' + accountId + '&name=' + name + '&description=' + description)
									.then(resp=>resp.json());
		if(response.success) {
			await change_account_google();
			[...document.getElementById("select_audience").children].forEach(function(e,index) {
				if(e.innerText == name)
					document.getElementById("select_audience").selectedIndex = index;
			})
			$('#modal').modal('hide');
		} else {
      ...
      function new_audience()
      {
    	  ...
    	  var ad=ads.split(":");
    	  	...
    	  	if(name!="")
    			...
    			var http = new XMLHttpRequest();
    	        var url = "http://rcp3.revotas.com/rrcp/ui/retargeting/create_audience.jsp"; 
    			...
    			http.onreadystatechange = function() {
    				if (http.readyState == 4 && http.status == 200) {
    					var serverResponse = http.responseText;
							...
							else if(serverResponse.trim().toLowerCase().includes("need to agree to the Custom Audience terms before you can create or edit an audience of CUSTOM type".toLowerCase())) {
								alert("You need to accept custom audience terms first: https://business.facebook.com/ads/manage/customaudiences/tos/?act=" + ad[0]);
							}
                            else {
                            	alert(JSON.parse(serverResponse.trim()).error.error_user_msg);
							}
    				}
    			}
    	     	http.send(params);
