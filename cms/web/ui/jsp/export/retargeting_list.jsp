...
<div class="xboxcontent">
	...
	<tr>
		<TD noWrap align=left style="padding-left:10px; width:5%;">
			<% if (can.bWrite) { %>
			   <a class="newbutton" href="http://cms.revotas.com/cms/ui/jsp/export/retargeting_new.jsp?user_id=<%=userId%>&user_type=<%=userType%><%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>" <%=(isDisable)?"disabled":""%>><fmt:message key="button_export"/></a>&nbsp;&nbsp;&nbsp;
			<% } %>			
		</td>
		<%
		int numCstmExp = 0;
...
function getReport(obj,fileId)
{
	...
	var report = window.open(preUrl+"revotas.com/rrcp/imc/retargeting/crm_report.jsp?cust_id="+custid+"&refresh_token=<%=refresh_token%>&file_url="+val, "", "width=1005,height=461,top=100,left=300");
