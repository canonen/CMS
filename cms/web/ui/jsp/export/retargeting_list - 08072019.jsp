...
String fbStatus         = request.getParameter("fbLogin");
...
<div class="xboxcontent">
	...
	<tr>
		<TD noWrap align=left style="padding-left:10px; width:5%;">
			<% if (can.bWrite) {
				 if(fbStatus.equals("logged")&& !fbUser.equals(""))
			   {
		     %>
			   <a class="newbutton" href="https://cms.revotas.com/cms/ui/jsp/export/retargeting_new.jsp?fbUser=<%=fbUser%>&fbStatus=<%=fbStatus%><%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>" <%=(isDisable)?"disabled":""%>><fmt:message key="button_export"/></a>&nbsp;&nbsp;&nbsp;
			<%} else { %>
				
              <a class="newbutton" onclick="fbErrorMessage()" ><fmt:message key="button_export"/></a>&nbsp;&nbsp;&nbsp;
		...
		<td noWrap align=right style="padding-right:10px;">
			...
			<a class="newbutton" href="retargeting_list.jsp?fbLogin=<%=fbStatus%>&fbUser=<%=fbUser%>"><fmt:message key="button_refresh"/></a>
		
		</td>
	</tr>
	</tbody>	
</table>
			<FORM  METHOD="POST" NAME="FT" ACTION="retargeting_delete.jsp?fbUser=<%=fbUser%>&fbStatus=<%=fbStatus%>"><INPUT TYPE="hidden" NAME="NDELS" VALUE="0" ><INPUT TYPE="hidden" NAME="FILE" VALUE="-9999">
			...
			{ 
				...
				%>
					...
					%>
					<td class="list_row<%= sClassAppend %>">
						<%=(nStatusID == ExportStatus.COMPLETE)?"<a href=\"retargeting_edit.jsp?fbUser="+fbUser+"&fbStatus="+fbStatus+"&file_id="+sFileId+"&categoryId="+sSelectedCategoryId+" \" >"+sFilename+"</a>":sFilename%>
					</td>
					<%} else {%>
					
					<td class="list_row<%= sClassAppend %>">
						<%=(nStatusID == ExportStatus.COMPLETE)?"<a onclick=\"fbErrorMessage()\" >"+sFilename+"</a>":sFilename%>
					</td>
