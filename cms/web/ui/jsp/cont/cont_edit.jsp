...
<%
	...
	String contName = "New Content", contStatus = "", sendType = "", contHTML = "", contText = "", contAOL = "", contMJML = "";
	String creator="",creationDate="",editor="",modifyDate="",firstPers="",firstBlock="";
	String unsubID="",unsubPosition="",textFlag="",htmlFlag="",aolFlag="",mjmlFlag = "";
	...
	try
	{
		...
		if (contID!=null)
		{
			...
			if (rs.next())
			{
				...
				b = rs.getBytes("MJML");
				contMJML = (b == null) ? "" : new String(b, "UTF-8");
				...
				mjmlFlag = rs.getString("send_mjml_flag");
		...
		sSql =
				" SELECT msg_id, ISNULL(msg_name,''), ISNULL(html_msg,''), ISNULL(text_msg,''), ISNULL(aol_msg,''),ISNULL(mjml_msg,'') " +
		...
		while (rs.next())
		{
			...
			String allConts = contText + contHTML + contAOL + contMJML;
...
<fmt:bundle basename="app">
	...
	<head>
		...
		<script>
			...
			if (isIE () == 7) {
				...
				sNew.src = "/cms/ui/js/CKEditor/ckeditor.js";
				var s0 = document.getElementsByTagName('script')[0];
				s0.parentNode.insertBefore(sNew, s0);
				var CKEDITOR_BASEPATH = '/cms/ui/js/CKEditor/';
			} else {
				document.write('<script type="text/javascript" src="/cms/ui/js/ckeditor_4.7.3_full/ckeditor/ckeditor.js"><\/script>');
			}


			var DragAndDropON=false;

			if((navigator.userAgent.indexOf("Opera") || navigator.userAgent.indexOf('OPR')) != -1 )  {DragAndDropON=true;   }
			else if(navigator.userAgent.indexOf("Chrome") != -1 ){DragAndDropON=true;   }
			else if(navigator.userAgent.indexOf("Safari") != -1){DragAndDropON=false; }
			else if(navigator.userAgent.indexOf("Firefox") != -1 ) { DragAndDropON=true; }
			/*
	...
	<SCRIPT LANGUAGE="JAVASCRIPT">
		...
		function AddLinkTable(evt)
		{
			...
			var newRow;
			var newCell;
			console.log(FT.EditLinkName.value);
		...
		function SubmitPrepare(Act)
		{
			...
			// 04-05-2018 Sinan  Contenteki &amp'i htmlini & �evirir.
			var editor_inst_html = CKEDITOR.instances.ContentHTML.getData();
			editor_inst_html=editor_inst_html.replace(/&amp;/g, "&").replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&quot;/g, "\"");
			CKEDITOR.instances.ContentHTML.setData(''+editor_inst_html);
			FT.ContentHTML.value=editor_inst_html ;
		...
		function dropTabs(CustID,ContID)
		{

			var ContentText = Base64.encode(FT.ContentText.value);
			var contTypeID 	= FT.contTypeID.value;
			var ContentName = Base64.encode(FT.ContentName.value);
			var Statuses 	= FT.Statuses.value;
			var SendTypes	= FT.SendTypes.value;
			var unsubID 	= FT.unsubID.value;
			var unsubPos 	= FT.unsubPos.value;
			var POSTData=CustID+"</>"+ContID+"</>"+ContentText+"</>"+contTypeID+"</>"+ContentName+"</>"+Statuses+"</>"+SendTypes+"</>"+unsubID+"</>"+unsubPos;

			var ENCODE=Base64.encode(POSTData);

			// window.location="http://localhost:70/#!/drop/"+ENCODE;
			window.location="https://cms.revotas.com:6999/#!/drop/"+ENCODE;
			// window.open("http://localhost:70/#!/drop/"+ENCODE);
	...
	<form name="FT" method="post" action="cont_save_zafer.jsp" style="display:inline;">
		<%
		...
		<table width="750px" class=listTable cellspacing=0 cellpadding=0>
			...
			<tr style="background-color:#F2F2F2">
				...
				<td class=Tab_OFF id="tab3_Step4" style="display:none;" width=100  valign=center nowrap onclick="dropTabs('<%=cust.s_cust_id%>', '<%=contID%>');" align=middle><b>Email Builder</b></td>
					...
					<table  cellspacing=1 cellpadding=2 width=100%>
						<tr>
							<td align="center">
								...
								<textarea rows="11" name="ContentText" cols="60" style="width: 80%; height: 231"><%= HtmlUtil.escape(contText) %></textarea>
								<br><br>
				...
				<td valign=top align=center width="100%" colspan=4>
					...
					<script type="text/javascript">
						...
						if (isIE () == 7) {
							...
							var CKEDITOR_BASEPATH = '/cms/ui/js/ckeditor_4.7.3_full/ckeditor/';
						}
					...
					<script type="text/javascript">
						...
						CKEDITOR.instances.ContentHTML.on( 'mode', function(ev) {
		...
		<script>
			if(DragAndDropON){
				var tabdoc=document.getElementById("tab3_Step4");
				tabdoc.style.display="block";
...
<%!
	...
	private String getLogicBlockListHtml(String sContId) throws Exception
	{
		...
		String sText = cb.s_text_part + cb.s_html_part + cb.s_aol_part + cb.s_mjml_part;
