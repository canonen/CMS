        ...
        while (rs.next())
        {
            ...
            else
            {
				...
				<script>
					...
					tbody.innerHTML = `<%=htmlContentChild%>`;
					contHTMLList['<%=contID%>'] = [...tbody.children].reverse();
...
<fmt:bundle basename="app">
    ...
    <head>
        ...
        <script language="javascript">
            ...
            {
				var element = contHTMLList[id][0];
				if(!element.getBoundingClientRect().width) {
					contHTMLList[id].forEach(element => {
						document.getElementById("link_" + id).parentElement.parentElement.insertAdjacentElement('afterend',element);
					});
				} else {
					contHTMLList[id].forEach(element=>element.remove());
				}
            }
