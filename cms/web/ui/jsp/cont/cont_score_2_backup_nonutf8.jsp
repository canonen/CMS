...
<style>
	...
	surrounder .theads {
		color:#3E434A;font-weight:bold;font-size:11px;
	}
	body {
...
<%
	...
	double hitts = Double.parseDouble(sf.hits());
	String str = "";
	
	if(hitts <= 3) {
		str = "SAFE";
	} else if (hitts > 3 && hitts <= 5) {
	
		str = "FAIRLY SAFE";
	} else if (hitts > 5 && hitts <= 7) {
		str = "CAUTION";
	} else {
		str = "HIGH RISK";
	}
...
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;" class="surrounder">
	<tr>
		<td colspan="2" style="background-color:#dadada;"><b>Score Overview</b></td>
	</tr>
	<tr>
		<td style="background-color:#f0f0f0;">
			<div class="theads">Total Score</div>
		</td>
		<td>
			<div style="margin-bottom:5px;"><span style="font-weight:bold;"><%= sf.hits() %>,</span> <span style="font-weight:bold;color:#2173AF;"> <%=str%></span></div>
			<div style="border:1px solid #cccccc;width:300px;height:20px;background-color:#DDDDDD;"><div style="background: url(http://www.revotas.com/alertbar.png) no-repeat scroll 0 0 transparent;border-right: 1px solid #cccccc;height: 20px;width: <%=(hitts < 0) ? 0 : hitts*10%>%;"></div></div>
		</td>
	</tr>
	<tr>
		<td style="background-color:#f0f0f0;"><div class="theads">Minimal Spam Score</div></td>
		<td><span style="font-weight:bold;"><%= sf.required() %></span></td>
	</tr>
	<tr>
		<td style="background-color:#f0f0f0;"><span class="theads">Is This Considered Spam?</span></td>
		<td style='background-color:#<%=(sf.isSpam())?"ed0600":"45f400"%>'><span style="font-weight:bold;color:black"><%=(sf.isSpam())?"Yes":"No"%></span></td>
	</tr>
</table>

<br>
<div style="text-align:left;font-size:11px;font-family:Tahoma;color:#636B75"><u>Disclaimer</u>: Revotas makes no representation concerning the accuracy of the scores or the implied recommendations provided herein. These are informational only and Revotas does not guarantee that your email will be delivered to its intended recipient.</div>
<br>

<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;" class="surrounder">
	<tr>
		<td colspan="3" style="background-color:#dadada;"><b>Score Detail</b></td>
	</tr>
	<tr>
		 <% if(report.equals("")) {
			out.println("<td colspan='3'>Detail is not available</td>");
		 } else {
			out.println(report);
		 }
		 %>
...
<br/>
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;" class="surrounder" width="100%">
	<tr>
		<td style="background-color:#dadada;"><b>Message</b></td>
	</tr>
</table>
<br/>
<table width=100% cellspacing=1 cellpadding=0>
  <tr><td><table class=main width=100% cellspacing=0 cellpadding=3 border=0>
            <tr><td><textarea cols=150 rows=25 wrap=hard>From: <%= URLDecoder.decode(from, "UTF-8") + "\n"%>Subject: <%= URLDecoder.decode(subj_text, "UTF-8") + "\n\n" %><%= previewText %></textarea>
            </td></tr>
          </table></td></tr>
</table> 
<%
...
<%	
	   	byte[] iso88591Data = subj_html.getBytes("ISO-8859-1");
    	String newS = new String(iso88591Data,"UTF-8");
...
<table width=100% cellspacing=0 cellpadding=0>
  <tr><td><table class=main width=100% cellspacing=1 cellpadding=3 border=0>
            <tr><td><b>From</b>: <%= URLDecoder.decode(from, "UTF-8") %><br><b>Subject</b>: <%= URLDecoder.decode(newS, "UTF-8") %><object width=100%><p><%= previewHtml %></object></td></tr>
          </table></td></tr>
</table> 
<%
	}
...
<table width=100% cellspacing=1 cellpadding=0>
  <tr><td><table class=main width=100% cellspacing=1 cellpadding=3 border=0>
            <tr><td><b>From</b>: <%= URLDecoder.decode(from, "UTF-8") %><br><b>Subject</b>: <%= URLDecoder.decode(subj_aol, "UTF-8") %><object width=100%><p><%= previewAol %></object></td></tr>
          </table></td></tr>
</table> 
<%
	}
