...
<HEAD>
	...
	<link rel="stylesheet" href="../bilginet/default.css" TYPE="text/css">
	 <script language="javascript" src="../../js/tab_script.js"></script>
	 <script language="javascript" src="../bilginet/jquery.js"></script>
	 <script language="javascript" src="../bilginet/jquery.iframe-auto-height.plugin.js"></script>
	 <script language="javascript" src="../bilginet/bootstrap-tooltip.js"></script>
	 <script language="javascript" src="../bilginet/bootstrap.min.js"></script>

	 <script language="javascript">	
	 
	function makesure() {
		if (confirm('Kampanyay? ba?latmak istedi?inize emin misiniz?')) {
			send();
		}
		else {
			return false;
		...
		else 
		{
			...
			if (null != CONT_ID)
			{
				if( !CONT_ID.equals(""))
				{
					%>
					if (stepNum == '3')	{
						window.frames["selectContent"].document.location.href = "/cms/ui/jsp/bilginet/ctm/pageedit.jsp?isEdit=true&contentID=<%= WizardID %>";
					}
					<%
				}
			}
...
<BODY ONLOAD="<%= (!can.bWrite)?"disable_forms();":"" %>">
	...
	<div id="wrapper">
		...
		<ul id="tabnav">
				...
				<li>
					<a class="" href="help.jsp?a=help">Yard?m</a>
				</li>
			</ul>
		...
		<tr>
			<td align="left">
				<a target="_blank" href="help.jsp?a=help" class="zbuttons zbuttons-medium zbuttons-light-gray">
					...
					<span class="zlabel">Yard?m</span>
				</a>
			</td>
			
			<%
			if(isDone) {
				out.println("<td align='center'><div style='font-size:11px;' class='error-box'>Bu kampanya gonderilmis durumda<div></td><td width='300px;'></td>");
			}	
			%>
				...
				if( !isDone && !isSending && !isTesting)
				{
					%>
						<a href="#" onclick="savenexit();" class="zbuttons zbuttons-medium zbuttons-blue">
							...
							<span class="zlabel">Kaydet &amp; Ç?k</span>
						</a>
					<%
				}
...
<% if( tmpType == 1 ) { %>
	<b>Bilgilendirme!</b> Test i?leminiz ba?lat?lm??t?r. Her hangi bir hatan?n olu?mamas? için test kampanyan?z? kontrol ettikten sonra
	gerçek kampanyan?z? ba?lat?n.
<% } else { %>
	<b>Bilgilendirme!</b> Kampanya i?leminiz ba?lat?lm??t?r.
<%	} %>
</div>
<%	} %>
...
<div id="block1_Step1" style="margin-top:5px;<%if (request.getParameter("step") != null) {out.println(";display:none;");}%>">
	<table cellpadding="0" cellspacing="0" width="100%" class="list-table noborder-p8">
		<tr>
			<td style="border-bottom:none" colspan="2" class="desc-texts">A?a??da ki kutucu?a kampanya ismini girin. Kampanyalar? ayr??t?rabilmek için kampanyan?za e?siz bir isim verin.</td>
		</tr>
		<tr>
			<td class="htexts" width="100">Kampanya Ad?</td>
...
<div id="block1_Step2" style="margin-top:5px;display:none;">
	<table cellpadding="0" cellspacing="0" width="100%" class="list-table noborder-p8">
		<tr>
			<td style="border-bottom:none" colspan="2" class="desc-texts">Kampanyan?z? hangi hedef gruba gönderece?inizi seçin.</td>
		</tr>
		<tr>
...
<div class="camp-section">
	<div class="camp-section-info">
		<div class="camp-section-header">Kampanya Ad?</div>
...
<div class="camp-section">
	...
	<div class="camp-section-header">Gönderen Ad?</div>
	<div class="camp-section-desc">Emaillerinizde gönderen ad? alan?nda <span><%=FROM_NAME%></span> gözükecektir.</div>
	<div style="clear:both"></div>
</div>
...
<%
	...
	rs = stmt.executeQuery("SELECT TOP 1 from_address_id, prefix+'@'+[domain] FROM ccps_from_address WHERE cust_id = "+cust.s_cust_id+" ORDER BY from_address_id DESC");
...
<div class="camp-section">
	...
	<%
		if(filterHtml_s4.equals(""))
			...
			out.println("Emailleriniz <span>"+filterHtml_s4+"</span> listesinden <span style='font-size:20px;color:#FF0000;'>"+ p_set_sendout_total + "</span> ki?iye gönderilecektir. <br><i>Kredinizin yetmedi?i durumda, kalan kredi kadar gönderim yap?lacakt?r.</i></span>");
	%>
...
<div class="camp-section">
	<div class="camp-section-info">
		<div class="camp-section-header">?çerik</div>
		<div class="camp-section-desc">
	<%
	...
	if(contentName.equals(""))
		out.println("<span style='color:#FF0000'>?çerik Olu?turulmad?</span>");
	else
		out.println("Email içeri?i olarak <span>"+contentName+"</span> gönderilecektir. ");
	%>
	<%
	if ((null != CONT_ID) && (!CONT_ID.equals(""))) 
	{
	%>
			?çeri?inizi önizlemek için <a href="javascript:previewContent('<%=sIframeSrc%>');">buraya</a> t?klay?n. 
	<%
	}
	%>
...
<div class="camp-section">
	<div class="camp-section-header">Email Ba?l???</div>
	<div class="camp-section-desc">
		Emailinizin konu ba?l???n? belirleyin.
		<div style="margin-bottom:5px;margin-top:10px;line-height: 20px;">
			...
			<label style="display:block;float:left">Email konusunun ba??n?nda özelle?tirilmi? isim kullan (örn: Onur Kaplan)</label>
			<div style="clear:both;"></div>
		</div>
	...
	%>		
		<a href="#" onclick="return makesure();" class="zbuttons zbuttons-normal zbuttons-green mta5">
			...
			<span class="zlabel"><%= (isHyatt)?"Onay bekliyor":"Kampanyay? Ba?lat" %></span>
		</a>
	<%
	}
	if(disableSending) {
		out.println("<div style='font-size:11px;' class='error-box'><b>Üzgünüz!</b> krediniz bu kampanyay? ba?latmak için yeterli de?il.</div>");
	}
	%>
</div>
<!-- Block Step 4 Content -->
...
<table id="Tabs_Table1" class="listTable" cellspacing="0" cellpadding="0" width="100%">	
	...
	<tr>
		<td style="padding: 5px;" valign="top" colspan="5">
			<ul id="tabnav2">
				<li><a id="tab6_Step1" class="active" onclick="toggleTabs('tab6_Step','block6_Step',1,3,'active','noClassPassiveTab');" href="javascript:void(0)">Kampanya Geçmi?i</a></li>
				<li><a id="tab6_Step2" class="noClassPassiveTab" href="javascript:void(0)" onclick="toggleTabs('tab6_Step','block6_Step',2,3,'active','noClassPassiveTab');">Test Geçmi?i</a></li>
				<li><a id="tab6_Step3" class="noClassPassiveTab" onclick="toggleTabs('tab6_Step','block6_Step',3,3,'active','noClassPassiveTab');" href="javascript:void(0)">Kullan?c? Geçmi?i</a></li>
			</ul>	
				...
				<tr>
					<td  valign=top align=center colspan=4>
						...
						%>
							...
							<tr>
								<td align="left" valign="middle" nowrap class="CampHeading"><b>Olu?turulma Tarihi</b></td>
								<td>&nbsp;&nbsp;</td>
					...
					<td valign=top align=center colspan=4>
						<table cellspacing="1" cellpadding="2" width="100%" border="0">
							<tr>
								...
								<td align="left" valign="middle" nowrap class="CampHeader"><b># S?raya Al?nan</b></td>
								<td align="left" valign="middle" nowrap class="CampHeader"><b># Gönderilen</b></td>
								<td align="left" valign="middle" nowrap class="CampHeader"><b>Olu?turulma</b></td>
								<td align="left" valign="middle" nowrap class="CampHeader"><b>Ba?lama</b></td>
								<td align="left" valign="middle" nowrap class="CampHeader"><b>Biti?</b></td>
							</tr>
						<%
							if( ORIGIN_CAMP_ID != null && !ORIGIN_CAMP_ID.equals("") && !ORIGIN_CAMP_ID.equals("null") )
							{
						...
						%>
							<tr>
								<td class="CampHeader" colspan="7">Bu kampanya için hiç test gönderilmemi?!</td>
							</tr>		
						<%
								}
							}
						...
						%>
							<tr>
								<td class="CampHeader" colspan="7">Kaydet butonuna bast?ktan sonra bu alan kampanya geçmi?ini gösterecektir.</td>
							</tr>
						<%
							}
						%>
				...
				<tr>
					<td  valign=top align=left colspan="4">
						<table  cellspacing="1" cellpadding="3" width="100%" border="0">
							<tr>
								<td class="CampHeader"><b>Olu?turan</b></td>
								<td><%= history[1] %></td>
								<td class="CampHeader"><b>En son güncelleyen</b></td>
								<td><%= history[3] %></td>
							</tr>
							<tr>
								<td class="CampHeader"><b>Olu?turulma tarihi</b></td>
								<td><%= history[0] %></td>
...
<INPUT TYPE="hidden" NAME="limit_per_hour" VALUE="500" style="display:none;">
<br><br>
</FORM>
</div>
