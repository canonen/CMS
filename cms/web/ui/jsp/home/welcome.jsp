...
$(document).ready( function() {
	...
	<% 
		if(cust.s_cust_id.equals("228"))
		{
	%>
			$.get(fileLoc+'5', function(data) {
				$("#form_submissions").html(' '+data);
			});
	<%
		}
	%>
			...
			$.get(fileLoc+'3', function(data) {
				...
				$.get(fileLoc+'4', function(data) {
					...
					$.get(fileLoc+'6', function(data) {
						$("#include6").html(data);
						totalEx=data;
						totalExPrc = (totalEx * 100)/totalDBSize;
						$("#include66").html(' (%'+totalExPrc.toFixed(2)+')');
...
function pieChart() {
	var chart = new Highcharts.Chart({
      ...
      title: {
         text: '<fmt:message key="account_summary"/>',
		 margin:0,
		 align: 'left',
		 x: 0,
		 y: 5
	 ...
	 {
	    				name: '<fmt:message key="dash_uns"/>',
	    				y: totalUnsubsPrc,
	    				color: '#AA4643'
	    			},
	    			{
	    				name: '<fmt:message key="dash_bou"/>',
	    				y: totalBBacksPrc,
	    				color: '#4D4E50'
			}
         ]
...
try
{
	...
	rs.close();
		...
		String campClass = "Tab_OFF";
		String reptClass = "Tab_ON";
					...
					<table cellpadding="4" cellspacing="0" width="100%">
						<tr>
							...
							<td style="border-bottom:1px solid #f1f0f0;">
								<div style="font-size:14px;font-family:Arial;color:#3c3c3c;margin-bottom:3px;"><fmt:message key="dash_total_db"/></div>
								<div>
						...
						<tr>
							...
							<td style="border-bottom:1px solid #f1f0f0">
								<div style="font-size:14px;font-family:Arial;color:#3c3c3c;margin-bottom:3px;"><fmt:message key="dash_unsub"/></div>
								<div>
						...
						<tr>
							<td><img src='/cms/ui/images/flag_2_icon&16.png'/></td>
							<td style="border-bottom:1px solid #f1f0f0">
								<div style="font-size:14px;font-family:Arial;color:#3c3c3c;margin-bottom:3px;">Exclude</div>
								<div>
									<span id="include6" style="font-size:16px;font-family:Arial;font-weight:bold;color:#ff6600">
										<img src="/cms/ui/images/smallloader.gif"/> 
									</span>
									<span id="include66" style="font-size:12px;font-family:Arial;color:#2b2b2b;"></span>
								</div>
							</td>
						</tr>
						<tr>
							...
							<td style="border-bottom:1px solid #f1f0f0">
								<div style="font-size:14px;font-family:Arial;color:#3c3c3c;margin-bottom:3px;"><fmt:message key="dash_bback"/></div>
								<div>
						...
						<tr>
							...
							<td style="border-bottom:1px solid #f1f0f0">
								<div style="font-size:14px;font-family:Arial;color:#3c3c3c;margin-bottom:3px;"><fmt:message key="dash_active"/></div>
								<div>
...
-->
		...
		<div style="float:left;width:385px;">			
			...
			<% 
				...
				{
			%>
				<div style="background-color: #ff6600;border: 1px solid #DDDDDD;color: #ffffff;font-family: arial;font-size: 14px;margin-bottom: 10px;padding: 5px;">
					Discount Form Recipient Count : <span style="font-weight:bold;font-size:16px;color:#fff" id="form_submissions"><img src="/cms/ui/images/smallloader.gif"/></span>
				</div>
			<%
				}
			%>
			<ul id="tabnav2">
				<li><a href="javascript:void(0);" class="active" id="tab2_Step1" onclick="toggleTabs('tab2_Step','block2_Step',1,2,'active','noClassPassiveTab');"><fmt:message key="Announcements"/></a></li>
				<li><a href="javascript:void(0);" class="noClassPassiveTab" id="tab2_Step2" onclick="toggleTabs('tab2_Step','block2_Step',2,2,'active','noClassPassiveTab');"><span><fmt:message key="user_notes"/></span></a></li>
			</ul>
											...
											<tr>
												<td>
													<div style="height:200px;overflow:auto;" class="flexcroll">
														<div style="font-weight:bold;color:#FF8821"><%= s_subject %></div>
														<div><%= s_body %></div>
													</div>
													<div style="margin-top: 5px;">
														<a href="javascript:loadSysNote('<%= sNoteId %>');" class="button_res"><fmt:message key="read_more"/></a>
														<a href="javascript:loadSysAnnounce();" class="button_res"><fmt:message key="past_announcements"/></a>
													</div>
												</td>
											</tr>
										</table>
...
<!-- recent assets start-->
						<%
							...
							<div>
								<ul id="tabnav2">
									<% if (canRept.bRead) { %><li><a id="tab1_Step2" class="active" onclick="toggleTabs('tab1_Step','block1_Step',2,3,'active','noClassPassiveTab');" href="javascript:void(0)"><fmt:message key="reports"/></a></li><% } %>
									<% if (canCont.bRead) { %><li><a id="tab1_Step3" class="noClassPassiveTab" href="javascript:void(0)" onclick="toggleTabs('tab1_Step','block1_Step',3,3,'active','noClassPassiveTab');"><fmt:message key="contents"/></a></li><% } %>
									<% if (canCamp.bRead) { %><li><a id="tab1_Step1" class="noClassPassiveTab" onclick="toggleTabs('tab1_Step','block1_Step',1,3,'active','noClassPassiveTab');" href="javascript:void(0)"><fmt:message key="campaigns"/></a></li><% } %>
								</ul>				
								...
								<table cellspacing="0" class=listTable cellpadding="0" width="100%" border="0">
									...
									<tr>
									<td>
										<% if (canCamp.bRead) { %><a target="_top" class="subactionbutton" href="../index.jsp?tab=Camp&sec=1"><fmt:message key="all_campaigns"/></a><% } %>
										<% if (canRept.bRead) { %><a target="_top" class="subactionbutton" href="../index.jsp?tab=Rept&sec=1"><fmt:message key="all_reports"/></a><% } %>
										<% if (canCont.bRead) { %><a target="_top" class="subactionbutton" href="../index.jsp?tab=Cont&sec=1"><fmt:message key="all_contents"/></a><% } %>
									</td>
									</tr>
								</table>
