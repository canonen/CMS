...
try
{
	...
	stmt = conn.createStatement();
		...
		if(custID==null)
		rs = stmt.executeQuery("select c.cust_id,c.cust_name, cs.camp_id, cc.camp_name from cque_schedule cs with(nolock) left join cque_campaign cc with(nolock) on cs.camp_id=cc.camp_id left join ccps_customer c with(nolock) on cc.cust_id=c.cust_id where start_date> getdate() - "+showDay+" and cc.type_id=2 and cc.status_id > 10 and cc.status_id <=60 order by cust_name asc");
		else
	...
	while (rs.next())
	{
		...
		out.println("<tr><td>"+custname+"</td><td>"+custid+"</td><td>"+campid+"</td><td>"+campname+"</td></tr>");
