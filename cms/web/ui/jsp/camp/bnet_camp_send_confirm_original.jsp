...
<BODY>
	...
	<div id="wrapper">
		...
		<div id="container">
			...
			<ul id="tabnav">
				<li>
					<a class="" href="../bilginet/home.jsp?a=home">Hesap Özeti</a>
				</li>
				<li>
					<a class="active" href="../bilginet/campaigns.jsp?a=campaigns">Kampanyalar</a>
				</li>
				<li>
					<a class="" href="../bilginet/reports.jsp?a=reports">Raporlar</a>
				</li>
				<li>
					<a class="" href="../bilginet/help.jsp?a=help">Yardım</a>
				</li>
			</ul>
			...
			<ul id="wizard-menu">
				<li class="stepli1 ">
					<a class="" id="step1" onclick="switchSteps('1');" href="javascript:void(null);">
						<span>Kampanya Adı</span>
					</a>
				</li>
				<li class="stepli2">
					<a class=" id="step2" onclick="switchSteps('2');" href="javascript:void(null);">
						<span>Hedef Grup</span>
					</a>
				</li>
				<li class="stepli3">
					<a class="" id="step3" onclick="switchSteps('3');" href="javascript:void(null);">
						<span>İçerik</span>
					</a>
				</li>
				<li class="stepli4">
					<a class="end current lastchild" id="step4" onclick="switchSteps('4');" href="javascript:void(null);">
						<span>Gönder</span>
					</a>
				</li>
			</ul>
			<a class="btn btn-small pabsoluter" onclick="javascript:stepUpNDown('up');" href="#">
				<i class="icon-arrow-right icon-black"></i> Devam Et</a>
					<a class="btn btn-small pabsolutel" onclick="javascript:stepUpNDown('down');" href="#">
		<i class="icon-arrow-left icon-black"></i> Geri Dön</a> 
		</div>

			<div class="camp-section">
	<div class="camp-section-header">Kampanya Adı</div>
	<div class="camp-section-desc"><%=HtmlUtil.escape(camp.s_camp_name)%></div>
...
if (!isPrintCampaign && (!bHasSampleset || bDisplayingFinal || (bDisplayingSamples && (sampleset.s_from_address_flag == null) || (sampleset.s_from_name_flag == null))))
{
	if (!bHasSampleset || bDisplayingFinal || (bDisplayingSamples && sampleset.s_from_name_flag == null) )
	{
		...
		<div class="camp-section">
			...
			<div class="camp-section-desc"><%= HtmlUtil.escape(msghdr.s_from_name) %></div>
			<div style="clear:both"></div>
		</div>
		<%
	}
			...
			if ( (msghdr.s_reply_to != null) && (!msghdr.s_reply_to.equals("")) )
			{
				%>
					<div class="camp-section">
						...
						<div class="camp-section-desc"><%= HtmlUtil.escape(msghdr.s_reply_to) %></div>
						<div style="clear:both"></div>
					</div>				
				<%
			}
...
if (!isPrintCampaign && (!bHasSampleset  || bDisplayingFinal || (bDisplayingSamples && sampleset.s_subject_flag == null) ))
{
	%>
		<div class="camp-section">
			...
			<div class="camp-section-desc"><%= HtmlUtil.escape(msghdr.s_subject_html) %></div>
			<div style="clear:both"></div>
		</div>	
	<%
}
if (!bHasSampleset  || bDisplayingFinal || (bDisplayingSamples && sampleset.s_cont_flag == null) )
{
	%>
		<div class="camp-section">
			...
			<div class="camp-section-desc"><%= HtmlUtil.escape(cont.s_cont_name) %></div>
			<div style="clear:both"></div>
		</div>
	<%
}
...
<div class="camp-section">
	...
	<div class="camp-section-desc"><%= HtmlUtil.escape(filter.s_filter_name) %></div>
	<div style="clear:both"></div>
</div>		
...
<a class="btn btn-small btn-info mt5" href="javascript:doSend(0);">
					<i class="icon-arrow-left icon-white"></i> Düzenlemeye Geri Dön</a> 
				...
				<% if (!bCantSendOffers) { %>
					...
					<% if (!bWorkflow) { %>
						...
						<a class="btn btn-small btn-info mt5" href="javascript:doSend(2);">
									<i class="icon-ok icon-white"></i>  Onayla ve Başlat</a>
					...
					<% } else if (bWorkflow && can.bApprove) { %>
						...
						<a class="actionbutton" href="javascript:doSend(2);">Confirm >> Send &amp; Approve Campaign</a>
							
					<% } else if ((bWorkflow && !can.bApprove) || (bWorkflow && templateRequiresApproval)) { %> 
						
						<a class="actionbutton" href="javascript:RequestApproval();">Confirm >> Request Approval</a>
...
function doSend(parm)
{
	...
	switch( parm )
	{
		case 0:
		{
			confirmation_form.action="../bilginet/wizard.jsp";
