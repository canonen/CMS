...
<table class="" cellspacing="0" cellpadding="0" width="100%">
	<tr>
		<td width="100" height="25">Testing List</td>
		...
		<td valign=center width=225 style="padding:10px;">
<%
	if( !isDone && !isSending && !isTesting && !isPending && can.bExecute && (!isPendingEdits || (isPendingEdits && isApprover)))
	{
%>
		<a class="actionbutton" href="javascript:send_test();">SEND A TEST ></a>
<%
	}
%>
		</td>
...
<% } %>
	...
	rs = stmt.executeQuery("SELECT max(camp_id) FROM cque_campaign with(nolock)"
...
<table class="" cellspacing="0" cellpadding="0" width="100%">
	...
	<tr>
		...
		if (sCalcCampID != null)
		{
			...
			rs = stmt.executeQuery("SELECT convert(varchar(255), finish_date, 100) FROM cque_camp_statistic with(nolock) WHERE camp_id = "+sCalcCampID);
...
<table class="" cellspacing="0" cellpadding="0" width="100%">
	...
	<tr>
		<table id=mapper width="100%" cellpadding=0 cellspacing=0 style="display:none"> 
			<tr> 
