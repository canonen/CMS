...
<table class="listTable" id="tab_<%=(++nTabId)%>" cellspacing="0" cellpadding="0" width="650" border="0">
	<tr>
		<td class="Tab_ON" id="tab_<%=nTabId%>_header_<%=(++nTabHeaderId)%>" width="150" onclick="switchSteps('tab_<%=nTabId%>', 'tab_<%=nTabId%>_header_<%=nTabHeaderId%>', 'tab_<%=nTabId%>_page_<%=nTabHeaderId%>');" valign="middle" nowrap align="center">Send Out</td>
	<%
	if (step4_tab2_show)
	{
		%>
		<td class="Tab_OFF" id="tab_<%=nTabId%>_header_<%=(++nTabHeaderId)%>" width="150" onclick="switchSteps('tab_<%=nTabId%>', 'tab_<%=nTabId%>_header_<%=nTabHeaderId%>', 'tab_<%=nTabId%>_page_<%=nTabHeaderId%>');" valign="middle" nowrap align="center">Advanced Options</td>
		<%
	}
	%>
	...
	<tr>
		<td class="Tab_GREY" valign="top" align="left" width="650"<%= step4_colspan %>><img height="2" src="../../images/blank.gif" width="1"></td>
	</tr>
