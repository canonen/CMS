...
<div class="page_header"><fmt:message key="cl_campaign_list_text"/></div>
<div class="page_desc"><fmt:message key="cl_create_txt"/></div>

<% if (showCamps) {out.println("showCamp1"+showCamp1); %>

<ul id="tabnav">
		
	<li><a href='#' <%= (!showCamp1)?" style=\"display:none;\"":"" %> onclick="FT.type_id.value = 2; FT.auto_queue_daily_flag.value = 0; GO(0);" class=" <%=("Standard".equals(sCampTypeLabel))?"active":""%>">Standard</a></li>
	<% if (!STANDARD_UI) { %><li><a href='#' <%= (!showCamp2)?" style=\"display:none;\"":"" %> onclick="FT.type_id.value = 3; FT.auto_queue_daily_flag.value = 0; GO(0);" class=" <%=("Send To Friend".equals(sCampTypeLabel))?"active":""%>">Send to friend</a></li><% } %>
	<li><a href='#' <%= (!showCamp3)?" style=\"display:none;\"":"" %> onclick="FT.type_id.value = 4; FT.auto_queue_daily_flag.value = 0; GO(0);" class=" <%=("Automated".equals(sCampTypeLabel))?"active":""%>">Automated</a></li>
	<li><a href='#' <%= (!showCamp4)?" style=\"display:none;\"":"" %> onclick="FT.type_id.value = 5; FT.auto_queue_daily_flag.value = 0; GO(0);" class=" <%=("Web/DM/Call".equals(sCampTypeLabel))?"active":""%>">Web/DM/Call</a></li>
</ul>
...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		<td class="listHeading" valign="center" nowrap align="left">
			...
			<table class="listTable" width="100%" cellspacing="0" cellpadding="2">
				<tr>
					...
					<th align="left" nowrap><fmt:message key="cl_column_campaign"/></th>
					<th align="left" nowrap><fmt:message key="cl_column_type"/></th>
		...
		out.println("s_camp_name: "+s_camp_name);
		out.println("s_status_name: "+s_status_name);
		out.println("s_status_id: "+s_status_id);
		out.println("s_type_id: "+s_type_id);
		out.println("s_type_id_name: "+s_type_id_name);
		 
		out.println("s_approval_flag: "+s_approval_flag);
		out.println("s_queue_daily_flag: "+s_queue_daily_flag);
		out.println("s_media_type_id: "+s_media_type_id);
		out.println("s_media_type_id_name: "+s_media_type_id_name);
		...
		<%
		out.println("canApprove.bExecute :"+canApprove.bExecute);
		if (canApprove.bExecute)
		{
			...
			<%	} else { 
					//CY Edit 10/17/2017 for SMS
					//if (!(s_media_type_id.equals("2") && (Integer.parseInt(s_status_id) == CampaignStatus.BEING_PROCESSED))) {
						out.println(CampaignStatus.BEING_PROCESSED);
						out.println("test"+Integer.parseInt(s_status_id));
					if (!(s_media_type_id.equals("1") && (Integer.parseInt(s_status_id) == CampaignStatus.BEING_PROCESSED))) {
					//if ((Integer.parseInt(s_status_id) == CampaignStatus.BEING_PROCESSED)) {
...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		<td class="listHeading" valign="center" nowrap align="left">
			...
			<div class="list-headers"><fmt:message key="cl_draft_campaigns_text"/></div>
		...
		<DIV id=xsnazzy>
			...
			<table class="listTable" width="100%" cellspacing="0" cellpadding="2">
				<tr>
					<th width="25%" align="left" nowrap><fmt:message key="cl_column_campaign"/></th>
					<th align="left" nowrap><fmt:message key="cl_column_type"/></th>
					<% if (canPrint) { %><th align="left" nowrap><fmt:message key="cl_draft_campaigns_text"/></th><% } %>
					<th width="25%" align="left" nowrap><fmt:message key="cl_column_modify_date"/></th>
					<th width="25%" align="left" nowrap><fmt:message key="cl_column_target_group"/></th>
					<th width="25%" align="left" nowrap><fmt:message key="cl_column_content"/></th>
				</tr>
	<%
	recAmount = 0;
	...
	<tr>
		<td class="listHeading" width="100%" valign="center" nowrap align="left">
			...
			<table class="listTable" cellspacing="0" cellpadding="0" border="0" align="right">
				<tr>
					<td align="right" style="font-weight:bold" valign="middle" nowrap>&nbsp;<span id="page_1"></span></td>
					<td align="center" valign="middle" >
						<table class="0" cellspacing="0" cellpadding="0" border="0">
							<tr>
				...
				<div class="list-headers"><fmt:message key="cl_completed_campaigns_text"/></div>
		...
		<DIV id=xsnazzy>
			...
			<table class="listTable" width="100%" cellspacing="0" cellpadding="2">
				<tr>
					...
					<th align="left" nowrap><fmt:message key="cl_column_target_group"/></th>
					<th align="left" nowrap><fmt:message key="cl_column_content"/></th>
					<th align="left" nowrap>Status</th>
...
%>		
				...
				<% 		if ((Integer.parseInt(s_status_id) == 60) && (!"Sample Set".equals(s_type_id_name)) && (!"Dynamic".equals(s_type_id_name))) { %>
							...
							<!--<a href="http://cms.revotas.com/cms/ui/jsp/report/report_object.jsp?act=VIEW&id=<%= s_camp_id %>" target="_top">View Report</a>-->
							<!--<a href="http://cms.revotas.com/cms/ui/jsp/report/report_redirect.jsp?act=VIEW&id=<%= s_camp_id %>">View Report</a>-->
				<% 		} else { %>
							&nbsp;
				<%		} %> 
					</td>
