...
try {
	...
	conn = cp.getConnection("wscamp/wscamp_list.jsp 1");
	...
	if (samount == null) samount = ui.getSessionProperty("wscamp_list_page_size");
	if ((samount == null)||("".equals(samount))) samount = "25";
	try { amount = Integer.parseInt(samount); }
	catch (Exception ex) { samount = "25"; amount = 25; }
	ui.setSessionProperty("wscamp_list_page_size", samount);
...
<table cellpadding="3" cellspacing="0" border="0" width="100%">
	<tr>
		...
		{
			%>
			<a class="newbutton" href="wscamp_edit.jsp?<%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>">New WS Campaign</a>&nbsp;&nbsp;&nbsp;
			<%
		}
		%>
		</td>
...
<FORM METHOD="GET" NAME="FT" ID="FT" ACTION="wscamp_list.jsp" style="display:inline;">
...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		<td class="listHeading" valign="center" nowrap align="left">
			...
			Draft Web Service Campaigns
			<br><br>
			<table class="listTable" width="100%" cellspacing="0" cellpadding="2">
				<tr>
					<th width="8%" align="left" nowrap>Web Service ID</th>				
					<th width="7%" align="left" nowrap>WS Status</th>
					<th width="20%" align="left" nowrap>Campaign</th>					
					<th width="15%" align="left" nowrap>Modify Date</th>
					<th width="20%" align="left" nowrap>Target Group</th>
					<th width="15%" align="left" nowrap>Content</th>
					<th width="15%" align="left" nowrap>Test Date</th>
					<th width="15%" align="left" nowrap>Start Date</th>
				</tr>
			<%
			String sSql = "usp_cxcs_ws_camp_list_get " + cust.s_cust_id + "," + sSelectedCategoryId;
			...
			while( rs.next() )
			{
				...
				sWsCampId =  rs.getString(4);
				sWsSealId =  rs.getString(5);
				sFilterId = rs.getString(6);
				sFilterName =  rs.getString(7);
				sContId = rs.getString(8);
				sContName =  rs.getString(9);
				sCreateDate = rs.getString(10);
				sModifyDate = rs.getString(11);
				sTestDate =  rs.getString(12);
				sStartDate =  rs.getString(13);
				sStatus = rs.getString(14);
				sWsStatusId = rs.getString(15);
				...
				%>
				<tr>
					<td class="listItem_Data<%= sClassAppend %>"><a href="wscamp_edit.jsp?ws_camp_id=<%=sWsCampId%>"><%=sWsCampId%></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%=sWsStatus%></td>
					<td class="listItem_Data<%= sClassAppend %>"><a href="../camp/camp_edit.jsp?camp_id=<%=sCampId%><%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>"><%=sCampName%></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%=sModifyDate%></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><a href="../filter/filter_edit.jsp?filter_id=<%=sFilterId%>"><%=sFilterName%></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><a href="../cont/cont_edit.jsp?cont_id=<%=sContId%>"><%=sContName%></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%=sTestDate%></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%=sStartDate%></td>
				</tr>
				<%
			}
			rs.close();
...
{ 
	ErrLog.put(this,ex,"wscamp_list.jsp",out,1);	
}
finally
{
