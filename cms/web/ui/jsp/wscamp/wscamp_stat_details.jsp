...
try
{
	...
	String sListFileCount = "n/a";
	String sImportFileCount = null;
	String sImportFileValidCount = null;
	String sImportFileImportCount = null; 
	
	sql = 
		"SELECT list_file_count FROM cxcs_ws_campaign" +
		" WHERE ws_camp_id = " + sWsCampId;
	rs = stmt.executeQuery(sql);
	if (rs.next()) {
		sListFileCount = rs.getString(1);
	}
	rs.close();
	...
	boolean bImportDone = false;
	if (sImportId != null && sImportId.length() > 0) {
		Import imp = new Import(sImportId);
		if (imp.s_status_id.equals(String.valueOf(ImportStatus.COMMIT_COMPLETE))) {
			bImportDone = true;
			sql =
				"SELECT imps.tot_file_recips, imps.tot_file_recips - imps.bad_emails, imps.tot_recips" +
				"  FROM cupd_import_statistics imps" +
				" WHERE imps.import_id = " + sImportId;
			rs = stmt.executeQuery(sql);
			if (rs.next()) {
				sImportFileCount = rs.getString(1);
				sImportFileValidCount = rs.getString(2);
				sImportFileImportCount =  rs.getString(3);
			}
			rs.close();		
		}
	}
	...
	sql = 
		"SELECT max(camp_id) FROM cque_campaign" +
		" WHERE type_id = " + CampaignType.TEST +
		"   AND status_id = " + CampaignStatus.DONE +
		"   AND mode_id = " + CampaignMode.CALC_ONLY +
		"   AND origin_camp_id = " + sCampId;
	rs = stmt.executeQuery(sql);
	if (rs.next()) {
		sCalcCampId = rs.getString(1);
	}
	rs.close();
	
	String sChildCampId = null;
	sql = 
		"SELECT max(camp_id) FROM cque_campaign" +
		" WHERE type_id = " + CampaignType.STANDARD +
		"   AND status_id >= " + CampaignStatus.READY_TO_SEND +
		"   AND origin_camp_id = " + sCampId;
	rs = stmt.executeQuery(sql);
	if (rs.next()) {
		sChildCampId = rs.getString(1);
	}
	rs.close();	
	...
	if (sChildCampId != null && sChildCampId.length() > 0) {
		csds.s_camp_id = sChildCampId;
		csds.retrieve();	
	}
	else if (sCalcCampId  != null && sCalcCampId.length() > 0) {	
	...
	if ("queue".equals(sAction)) {
		stepDesc = "Import and Queued Count Details";
	}
	else {
		stepDesc = "Import and Calculated Recipient Statistics";
	}
%>
<HTML>
<HEAD>
...
<BODY>
<FORM METHOD="POST" NAME="FT" ACTION="wscamp_stat_details.jsp" TARGET="_self">
<!--- Header----->
<table width=100% class=main cellspacing=0 cellpadding=0>
	<tr>
		<td class=sectionheader>&nbsp;<b class=sectionheader>WS Campaign:</b> <%= stepDesc %></td>
	</tr>
</table>
<br>
<!---- Info----->
			...
			<table class=main cellspacing=1 cellpadding=2 width="100%">			
				<tr>
					<td align="center" valign="middle" style="padding:10px;">
						<table class="main" cellspacing=0 cellpadding=3 border=0 width="100%">
							<tr>
								<th colspan="2">Step 0: Import Statistics</th>
							</tr>
							<tr>
								<td class="listItem_Data_Alt" align=left>Recipients in List File</td>
								<td class="listItem_Data_Alt" align=right><%= sListFileCount %></td>
							</tr>
<%					if(!bImportDone) { %>	
							<tr>
								<td class="listItem_Data" colspan="2">Import statistics not available</td>
							</tr>
<%                  } else { %>			
							<tr>
								<td class="listItem_Data_Alt" align=left>Recipients after scrub</td>
								<td class="listItem_Data_Alt" align=right><%= sImportFileCount %></td>
							</tr>
							<tr>
								<td class="listItem_Data_Alt" align=left>Recipients with valid emails</td>
								<td class="listItem_Data_Alt" align=right><%= sImportFileValidCount %></td>
							</tr>
							<tr>
								<td class="listItem_Data_Alt" align=left>Recipients imported</td>
								<td class="listItem_Data_Alt" align=right><%= sImportFileImportCount %></td>
							</tr>
							<tr>
...
<%					if (csds.size() == 0) { %>
							<tr>
								<th colspan="2">Step 1: Target Group Calculations</th>
							</tr>
							<tr>
								<td class="listItem_Data" colspan="2">Recipient statistics not available</td>
							</tr>
<% 					}
					else {
						String sClassAppend = "";
						...
						if (bDomainCount) {
							...
							<tr>
								<td class="listItem_Data<%= sClassAppend %>" align=middle colspan=2>
									...
									<input type=hidden name="ws_camp_id" value="<%= sWsCampId %>"/>
						...
						if (sWsSentCountFlag != null && sWsSentCountFlag.equals("1")) {
							...
							// use campaign creation date as sentDate
							java.util.Date campStartDate = null;
							rs = stmt.executeQuery("SELECT start_date FROM cque_schedule WHERE camp_id = " + sChildCampId);
							if (rs.next()) {
								campStartDate = rs.getDate(1);
							}
							rs.close();
							if (campStartDate == null) {
								rs = stmt.executeQuery("SELECT create_date FROM cxcs_ws_campaign WHERE ws_camp_id = " + sWsCampId);
								if (rs.next()) {
									campStartDate = rs.getDate(1);
								}
								rs.close();
							}
							logger.info("sentDate [campCreateDate] = " + campStartDate);
							try {
...
{ 
	ErrLog.put(this,ex, "Problem with wscamp_stat_details.",out,1);
}
finally
{
