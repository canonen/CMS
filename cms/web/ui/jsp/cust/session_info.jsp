...
<style>
	body{
		margin:0;background-color: #2F2F2F;
	}
.topNav a {
...
<div style="background:url(https://cms.revotas.com/cms/ui/ooo/images/newtopnavibg.gif);height:25px;border-bottom: 2px solid #000">
	<div style="float:right;font-family:Tahoma;font-size:11px;color:#FFF;margin-right: 20px;margin-top: 5px;">Welcome <%= user.s_user_name %> <%= (user.s_last_name!=null)?user.s_last_name:"" %>
	(<% if (hasChildren) { %><a href="#" onclick="switch_cust();">[Switch]</a><% } %><%=cSuper.s_cust_name%>)
	
	 | <span>Help</span> | <span>Logout</span>
	</div>

</div>
<div style="padding-bottom: 10px !important;padding-bottom: 12px;border-bottom:1px solid #ff8a24">
...
<table width="100%" cellspacing="0" cellpadding="0" border="0">
		<col>
		<tr>
			<td style="border-right:1px solid #000000;padding-right:10px;" width="70%" class="gradbg_thin" align="right" valign="middle">
				dsadsadasWelcome <b><%= user.s_user_name %> <%= (user.s_last_name!=null)?user.s_last_name:"" %></b>
			</td>
			
			<td style="border-right:1px solid #000000;padding-right:10px;padding-left:0 !important;padding-left:10px;" nowrap width="5%" class="gradbg_thin" align="right" valign="middle">
				Company <% if (hasChildren) { %> <a href="#" onclick="switch_cust();">[Switch]</a><% } %>:
				<b><%=cSuper.s_cust_name%></b>
			</td>
		<%
		if (hasChildren)
		{
			if(cSuper != cActive)
			{
				%>
				 &gt;&gt;
				<b><%= cActive.s_cust_name %></b>
				<%
			}
		}
		%>
		
		<td width="5%" style="padding-left:10px;padding-right:10px;" nowrap class="gradbg_thin">Logout</td>		
		</tr>
	</table>
	-->
<iframe width="1" height="1" name="cust_tree" id="cust_tree" src="cust_tree.jsp" style="display:none;"></iframe>
</body>
</html>
	<%
}
