...
function filter_part_delete(obj)
{
	<% if (bCanEditParts) { %>
          ...
          filter_part.parentElement.removeChild(filter_part);
     <% } else { %>
          return
     <% } %>
}
...
function filter_part_select_filter()
{
	<% if (bCanEditParts) { %>
          lastSrcElement = event.srcElement;
...
function filter_part_add_formula()
{
	<% if (bCanEditParts) { %>
     	filter_part_add_part(formula_prototype);
     <% } else { %>
          return
     <% } %>
}

function filter_part_add_part(obj)
{
	filter_part_add(obj.innerHTML)
}

function filter_part_add(html)
{
	<% if (bCanEditParts) { %>
          element = lastSrcElement;
          if(event!=null) element = event.srcElement;

          filter = getParentByXmlTag(element, 'filter');
          filter_parts = getChildByXmlTag(filter, 'filter_parts');
          filter_parts.innerHTML += html;
...
function filter_parts_inex(obj)
{
	<% if (bCanEditParts) { %>
          element = event.srcElement;
          filter = getParentByXmlTag(element, 'filter');
...
function filter_parts_group(obj)
{
	<% if (bCanEditParts) { %>
          element = obj;
...
function filter_parts_ungroup(obj)
{
	<% if (bCanEditParts) { %>
          element = event.srcElement; 
          filter = getParentByXmlTag(element, 'filter');
...
function getParentByXmlTag(obj, parent_name)
{
	...
	for(oParent=obj.parentElement; oParent!=null; oParent=oParent.parentElement)
	{
		//alert(oParent.tagName);
		if(oParent.xml_tag == parent_name)
		{
			oResult = oParent;
			break;
		}
...
function getChildByXmlTag(obj, child_xml_tag)
{
	...
	for(var i=0; i<l; ++i)
	{
		...
		if(oChild.xml_tag == null)
			oChild = getChildByXmlTag(oChild, child_xml_tag);

		if((oChild!=null)&&(oChild.xml_tag == child_xml_tag))
		{
			oResult = oChild;
			break;
		}
