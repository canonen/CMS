...
<%
    ...
    String sFilterXml = "";
    String sFilterParamsXML = "";
    String sFilterPartsXML = "";
    String sPreviewAttrsXML = null;
    String main_filter_id = "";
    String main_filter_name = "";
    String main_type_id = "";
    String main_cust_id = "";
    String main_status_id = "";
    String main_usage_type_id = "";
    String jsonString = "";
    String testXML = "";
    String LFilterId="";
...
/*    FilterPartSQL =
            " SELECT child_filter_id," +
                    ...
                    " WHERE parent_filter_id = '" + sFilterId + "'";
    rs2 = stmt2.executeQuery(FilterPartSQL);*/

    //testXML
String ttt=childFilter(sFilterId);
    //com.britemoon.cps.tgt.Filter sFilterMain = childFilter(sFilterId);

out.println("ttt: " + ttt);



    //------------------------------------FilterParts XML olusturma--------------------------------------

    while (rs2.next()) {
        com.britemoon.cps.tgt.Filter sfilter = new com.britemoon.cps.tgt.Filter(rs2.getString(1));
        Formula sFormula = new Formula(rs2.getString(1));
        sfilter.m_Formula = sFormula;
        FilterPart filterPart = new FilterPart();
        filterPart.m_ChildFilter = sfilter;
        filterPart.s_display_seq = rs2.getString(2);
        filterPart.retrieve();

        sFilterPartsXML = sFilterPartsXML + filterPart.toXml();
    }
    sFilterPartsXML = "<filter_parts>" + sFilterPartsXML + "</filter_parts>";
    //------------------------------------FilterParams XML olusturma--------------------------------------

    FilterParams filterParams = new FilterParams();
    filterParams.s_filter_id = sFilterId;
    filterParams.retrieve();
    sFilterParamsXML = filterParams.toXml();
    sFilterParamsXML = sFilterParamsXML.replaceAll("<__cdata>", "<![CDATA[").replaceAll("</__cdata>", "]]>").replaceAll("<__cdata/>", "<![CDATA[NOP]]>");
    sFilterParamsXML = sFilterParamsXML.replaceAll("<filter_id>.*?</filter_id>", "");
    sFilterParamsXML = sFilterParamsXML.replaceAll("<!\\[CDATA\\[BOOLEAN\\+OPERATION\\]\\]>", "<![CDATA[BOOLEAN OPERATION]]>");

    //------------------------------------PreviewAttrs XML olusturma--------------------------------------
    PreviewAttrs previewAttrs = new PreviewAttrs();
    previewAttrs.s_filter_id = sFilterId;
    previewAttrs.retrieve();
    sPreviewAttrsXML = previewAttrs.toXml();

    //------------------------------------Filter XML olusturma--------------------------------------------

    main_filter_id = sFilterMain.s_filter_id;
    main_filter_name = sFilterMain.s_filter_name;
    main_type_id = sFilterMain.s_type_id;
    main_cust_id = sFilterMain.s_cust_id;
    main_status_id = sFilterMain.s_status_id;
    main_usage_type_id = sFilterMain.s_usage_type_id;

    sFilterXml =
            "<filter_id>" + main_filter_id + "</filter_id>" +
                    "<filter_name>" + main_filter_name + "</filter_name>" +
                    "<type_id>" + main_type_id + "</type_id>" +
                    "<cust_id>" + main_cust_id + "</cust_id>" +
                    "<status_id>" + main_status_id + "</status_id>" +
                    "<usage_type_id>" + main_usage_type_id + "</usage_type_id>" +
                    sFilterPartsXML + sFilterParamsXML + sPreviewAttrsXML;
    sFilterXml = "<filter>" + sFilterXml + "</filter>";
...
%>
<%!
public String childFilter(String filterId)
{
    FilterPartLocalSQL =
            " SELECT child_filter_id," +
                    " display_seq " +
    ...
    rs2 = stmt2.executeQuery(FilterPartLocalSQL);

    while (rs2.next()) {
        LFilterId=rs2.getString(1);
        com.britemoon.cps.tgt.Filter sfilter = new com.britemoon.cps.tgt.Filter(LFilterId);
        System.out.println("LFilterId : " + LFilterId);
        childFilter(rs2.getString(1));
    }
...
filterPart.retrieve();
sFilterPartsXML = sFilterPartsXML + filterPart.toXml();
testXML= sFilterPartsXML + filterPart.toXml();
--%>
