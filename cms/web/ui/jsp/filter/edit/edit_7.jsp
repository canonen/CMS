...
<HEAD>
	<TITLE><%= sTargetGroupDisplay %>: Recipients who were in a specific Import</TITLE>
	...
	<SCRIPT>
		...
		function setFilterName(obj)
		{
			...
			filter_form.filter_name.value = '(IMPORT) ' + ops[si].text;
			display_name.innerText = "(IMPORT) " + ops[si].text;

			id_val = ops[si].value;
			if((id_val == null)||(id_val == '')) filter_form.save.disabled = true;
			else  filter_form.save.disabled = false;
		}
	</SCRIPT>
</HEAD>
		...
		%>
				<tr>
					<td align="center" valign="middle" style="padding:10px;">
						Recipients who were in a specific Import<br>
						Select an Import:<br>
						...
						while (rs.next())
						{
							...
							%>
								<option value="<%=sId%>"<%=((sId.equals(sImportId))?" selected":"")%>><%= HtmlUtil.escape(sName) %></option>
							<%
						}
						rs.close();
						%>
			...
			<table class=main cellspacing=1 cellpadding=2 width="100%">
				<tr>
					<td align="center" valign="middle" style="padding:10px;">
						...
						<input type="submit" class="subactionbutton" name="save" value="Save >>">&nbsp;&nbsp;
					</td>
				</tr>
			</table>
		</td>
