...
<HEAD>
	<TITLE><%= sTargetGroupDisplay %>: Recipients opted-in to a Newsletter</TITLE>
	...
	<SCRIPT>
		...
		function setFilterName(obj)
		{
			...
			filter_form.filter_name.value = '(NEWSLETTER) ' + ops[si].text;
			display_name.innerText = "(NEWSLETTER) " + ops[si].text;
		...
		%>
				<tr>
					<td align="center" valign="middle" style="padding:10px;">
						Recipients opted-in to a Newsletter<br>
						...
						while (rs.next())
						{
							...
							%>
								<option value="<%=sId%>"<%=((sId.equals(sAttrId))?" selected":"")%>><%= HtmlUtil.escape(sName) %></option>
							<%
						}
						rs.close();
						%>
