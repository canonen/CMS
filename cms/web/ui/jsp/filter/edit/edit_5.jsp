...
<HEAD>
	<TITLE><%= sTargetGroupDisplay %>: Recipients who clicked on a specific link</TITLE>
	...
	<SCRIPT>
		...
		function setFilterName(obj)
		{
			...
			filter_form.filter_name.value = '(LINK) ' + ops[si].text;
			display_name.innerText = "(LINK) " + ops[si].text;
		...
		%>
				<tr>
					<td align="center" valign="middle" style="padding:10px;">
						Recipients who clicked on a specific link<br>
						Select a Link:<br>
						...
						while (rs.next())
						{
							...
							%>
								<option value="<%=sId%>"<%=((sId.equals(sLinkId))?" selected":"")%>><%= HtmlUtil.escape(sName) %></option>
							<%
						}
						rs.close();
						%>
