...
function filter_part_delete(obj,event)
{
	<% if (bCanEditParts) { %>
          ...
          filter_part = getParentByXmlTag(obj, 'filter_part');
          
          
         
          
          
          filter_part.parentNode.removeChild(filter_part);
     <% } else { %>
          return
     <% } %>
}
...
function filter_part_select_filter(event)
{
	<% if (bCanEditParts) { %>
          ...
          URL = "select/select_zaf.jsp?usage_type_id=<%= sUsageTypeId %>";
          SmallWin = window.open(URL, windowName, windowFeatures);
     <% } else { %>
          return
     <% } %>
...
{
	filter_part_add(obj.innerHTML,event)
}
	...
	<% if (bCanEditParts) { %>
          ...
          for(var i=1; i<l; ++i)
          {
          	...
          	//alert('NUMBER  '+i);
               ...
               if(group_ungroup_checkbox.checked == true || group_ungroup_checkbox.checked || group_ungroup_checkbox.value == 'on')
               {
               	//alert('NUMBER  '+i+' part 4 ');
               
               		docContainer += filter_part.innerHTML;
               		filter_parts_old.removeChild(filter_part);
          ...
          filter_parts_new.innerHTML =docContainer;
     <% } else { %>
          return
     <% } %>
}
