...
String MonthlyGrowth = request.getParameter("MonthlyGrowth");
 if(tarih_aralik!=null){
		...
		d_startdate = parts[0] + " 00:00:00";
		d_enddate = parts[1] + " 23:59:59";
}
 		...
 		<div class="col-md-12">
					...
					<div class="box box-primary">
								...
								<div id="graph-div" class="box-body">
												<form method="post" action="report_smartwidget_activity_day_new_iframe.jsp?cust_id=<%= sCustId%>&popup_id=<%=popupId%>">
													<div class="row">
																		<div class="col-xs-6">
...
document.getElementById('total-view').innerText = popupStatistics[popupId].totalView;
document.getElementById('total-click').innerText = popupStatistics[popupId].totalClick;
document.getElementById('total-submit').innerText = popupStatistics[popupId].totalSubmit;
currencyFetched.then(()=>{
	document.getElementById('total-revenue').innerText = formatCurrency(popupStatistics[popupId].totalRevenue, currencyConfig);
});
...
document.getElementById('graph_list').addEventListener('change',function(e) {
	window.location = 'report_smartwidget_activity_day_new_iframe.jsp?cust_id=' + <%=sCustId%> + '&popup_id=' + e.target.value;
});
...
(async function () {
	...
	var tempStartDate = tarihAralik !== 'null' ? tarihAralik.split('-')[0].trim() : moment().subtract(29, 'days');
	var tempEndDate = tarihAralik !== 'null' ? tarihAralik.split('-')[1].trim() : moment();
   $('#tarih_aralik').daterangepicker({
	   ranges: {
		...
		if(popup_Id === 'total_numbers')return;
    ...
    for(popupKey in popupStatistics) {
		...
		tr += '<td class="list_row" nowrap><a href="https://cms.revotas.com/cms/ui/smartwidgets/main.jsp?cust_id=<%=sCustId%>&popup_id='+popupKey+'">'+popupTemp.popupName+'</a></td>';
