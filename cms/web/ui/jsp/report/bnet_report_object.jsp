...
%>
<%@ include file="../mini/wvalidator.jsp"%>
...
try
{
	...
	String XSLFile = XSLDir+"ReportViewWizard.xsl";
	...
	if (lonely)
	{
		...
		rs = stmt.executeQuery(
			" SELECT 1 FROM cque_campaign with(nolock)" +
...
// <Optouts>          - Number of Opt-outs
		...
		if ("1".equals(sCache))
		{
			rs = stmt.executeQuery("SELECT distinct convert(varchar(30),c.cache_start_date,100), convert(varchar(30),c.cache_end_date,100),"
					...
					+ " FROM crpt_camp_summary_cache c with(nolock)"
					+ " LEFT OUTER JOIN ccps_cust_attr a c with(nolock) ON a.attr_id = c.attr_id"
					+ " LEFT OUTER JOIN ctgt_compare_operation o c with(nolock) ON o.operation_id = c.attr_operator"
