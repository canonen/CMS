...
try
{
	...
	//Taftuf
	//--------------------GeoLocation XML entegre...-------------------------------------
	//Prepared statement ile yazilacak TODO.....
	String campanyIdFromReport = null;
	campanyIdFromReport = request.getParameter("id");
	rs = stmt.executeQuery("SELECT country_code, country_name, read_count FROM czx_geolocation Where camp_id = " + campanyIdFromReport);	
	sXML += "<GeoLocation>";
	while( rs.next() ){
			
		sXML += "<Location>";
		
			sXML += "<country_code>"+rs.getString(1)+"</country_code>";
			sXML += "<country_name>"+rs.getString(2)+"</country_name>";
			sXML += "<count>"+rs.getInt(3)+"</count>";
	...
	rs = stmt.executeQuery("SELECT agent, read_count FROM czx_agentreadinfo Where camp_id = " + campanyIdFromReport);
	sXML += "<Agent>";
	while( rs.next() ){
		
		sXML += "<Brand>";
		
			sXML += "<agent_name>"+rs.getString(1)+"</agent_name>";
			sXML += "<agent_read_count>"+rs.getInt(2)+"</agent_read_count>";
	...
	String XSLFile = XSLDir+"ReportViewTayfun.xsl";
