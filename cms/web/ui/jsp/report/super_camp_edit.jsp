...
<table width=100% class=listTable cellspacing=0 cellpadding=0>
	<tr>
		<th class=sectionheader>&nbsp;<b class=sectionheader>Step <%=(nStep++)%>:</b> Choose super campaign name</th>
	</tr>
...
<table width=100% class=listTable cellspacing=0 cellpadding=0>
	<tr>
		<th class=sectionheader>&nbsp;<b class=sectionheader>Step <%=(nStep++)%>:</b> Select campaigns for super campaign</th>
	</tr>
			...
			<table cellspacing=0 cellpadding=0>
				<TR> 
					...
					<TD WIDTH="90" VALIGN="MIDDLE" ALIGN="CENTER" nowrap>
						...
						<br>
						<p><a class="subactionbutton" href="javascript:getSelected();">Move Left</a></p>
						<p><a class="subactionbutton" href="javascript:removeField();">Move Right</a></p>
					</TD> 
					<TD WIDTH="285" VALIGN="MIDDLE" ALIGN="LEFT">
						<SELECT multiple="multiple" id="source"  NAME="source" SIZE="15" STYLE="width: 285; height: 315" onDblClick="addField()"></SELECT>
					</TD> 
				</TR> 
			</TABLE>
		</td>
	 ...
	 var choose_val=[];
	  ...
	  for (var i = 0; i < x.options.length; i++) {
		  if(x.options[i].selected){
			  choose_val.push(x.options[i].value);
			  addMultipleField(x.options[i].text,x.options[i].value);
		     
	      }
	  }
	  
	  for(var j=0;j<choose_val.length;j++)
		  {
		       var y=document.getElementById("source");
		         for (var k = 0; k < y.options.length; k++) {
			        if(y.options[k].value==choose_val[j]){
			        	
			        	FT.source.options[k] = null;
...
function addMultipleField(a,b) {

	//if( FT.source.selectedIndex == -1 ) return false;

	FT.target.options[FT.target.length] = new Option (a,b);
}
