    ...
    try {
        ...
        if(nodeList !=null) {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                data.put("totalAmt", sumAmt);
                data.put("avgAmt", avgAmt);
                data.put("avgQty", avgQty);


                productData.put(data);
            }
            if (rowCount != 0) {
                if (totalAvgAmt != 0){totalAvgAmt = totalAvgAmt / rowCount;}

                   data.put("totalAvgAmt", totalAvgAmt);

                if (totalAvgQty != 0){totalAvgQtyDom = totalAvgQty / rowCount;}

                data.put("totalAvgQtyDom", totalAvgQtyDom);
            }
           
            salesPerformance.put(productData);
        }
        statement.close();

        out.print(salesPerformance.toString());
