...
function populate(objForm,selectIndex,Ind) {
	...
	var ay=objForm.month[Ind].options[objForm.month[Ind].selectedIndex].value;
	//month1 = (months.indexOf(ay));
	var deg;
	for( var i=0;i<months.length;i++)
	 ...
	 timeA = new Date(objForm.year[Ind].options[objForm.year[Ind].selectedIndex].value,deg+1 ,1);
	...
	objForm.day[Ind].options[0].setAttribute("selected", "selected");
...
timeC = new Date();
        ...
        for(var i=0;i<months.length;i++)
        	{
        	objForm.month[Ind].options[i]=new Option(months[i],months[i])
        	}
  ...
  // console.log(objForm.month);
	objForm.month[Ind].options[timeC.getUTCMonth()].setAttribute("selected", "selected");
	//objForm.month(Ind).options[timeC.getUTCMonth()].setAttribute("selected", "selected");
}
// Fill years: Start with current year and next 5 years
function getYears(objForm, Ind) {
	//alert(objForm.current_time.value);
	//timeC = new Date(objForm.current_time.value).toDateString();
	
	timeC = new Date();
	 
	currYear = timeC.getUTCFullYear();
	for (var i = -2; i < 3; i++) {
	 objForm.year[Ind].options[i+2]=new Option(currYear +i ,currYear +i);
	 }
	// console.log(objForm.year[Ind].options[3].setAttribute("selected", "selected"));
	 objForm.year[Ind].options[2].setAttribute("selected", "selected");
	//objForm.year(Ind).options[3].selected=true;
	getMonths(objForm,Ind);
	populate(objForm,1,Ind);
	
	
	objForm.day[Ind].options[timeC.getUTCDate()-1].setAttribute("selected", "selected");
...
function SetDateTime(objForm) {
  ...
  if( dateStart.getUTCFullYear() - timeC.getUTCFullYear() >= -1 ) {
