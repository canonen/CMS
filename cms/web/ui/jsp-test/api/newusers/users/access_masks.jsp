...
%>
<%=(!can.bWrite || (bWorkflow && iStatusId == UserStatus.PENDING_APPROVAL))?"onload='disable_forms();'":""%>>
<%
	if(can.bWrite)
	{
		...
		if (!bWorkflow ||
			...
			if(sUserId != null)
			{
				...
				out.println(u.s_user_id);

				if(sUserId!=null) data.put("userID",sUserId);
				else data.put("userID","0");

				data.put("sAprvlRequestId",sAprvlRequestId);

			   if(sUserId==null) data.put("user","new user");
			   ...
			   array = new JsonArray();
						...
						try
						{
							...
							while (rs.next())
							{
								...
								if((AccessRight.READ & iMask) == AccessRight.READ)data.put("AccessRightRead","checked");
								else data.put("AccessRightRead","");
								if((AccessRight.WRITE & iMask) == AccessRight.WRITE)data.put("AccessRightWRITE","checked");
								else data.put("AccessRightWRITE","");
								if((AccessRight.EXECUTE & iMask) == AccessRight.EXECUTE)data.put("AccessRightEXECUTE","checked");
								else data.put("AccessRightEXECUTE","");
								if((AccessRight.DELETE & iMask) == AccessRight.DELETE)data.put("AccessRightDELETE","checked");
								else data.put("AccessRightDELETE","");
								if((AccessRight.APPROVE & iMask) == AccessRight.APPROVE)data.put("AccessRightAPPROVE","checked");
								else data.put("AccessRightAPPROVE","");
									array.put(data);
								k++;
							}
							rs.close();
