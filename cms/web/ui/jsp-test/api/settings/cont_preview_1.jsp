...
<%
	...
	try
	{
		...
		for (int i=0;i<vPers.size();++i)
		{
			...
			data.put("oneAttrName",oneAttrName);
			data.put("oneAttrNamesArray",sArray);
			aster = "";
			data.put("aster",aster);

			if (oneAttrName.equals("RecipID") || oneAttrName.equals("recip_key"))
			{
				oneInternalField = true;
				aster = "*";
				data.put("oneInternalField",oneInternalField);
				data.put("aster",aster);
			}
			data.put("htmlValues",aster+sArray[1]);
			data.put("htmlValues",sArray[0]);
			array2.put(data);
		}
...
// System.out.println("sLogicBlockId = " + sLogicBlockId);
			...
			while (rs.next())
			{
				...
				data.put("tmpLogicID",tmpLogicID);
				data.put("tmpLogicName",tmpLogicName);
				data.put("tmpContID",tmpContID);
				data.put("tmpContName",tmpContName);
				data.put("tmpFilterID",tmpFilterID);
				data.put("htmlPart",htmlPart);
				data.put("textPart",textPart);
				data.put("aolPart",aolPart);
// System.out.println("tmpFilterID = " + tmpFilterID);
		...
		logger.info("filterIDs = "+filterIDs);
