...
//UI Type
    ...
    Statement stmt =null;
    ResultSet rs=null;
    JSONObject toJsonTMP =new JSONObject();
    ...
    try
    {
        ...
        if (request.getParameter("id")!=null)
        {
            ...
            for (int i=0; i < sTempCampList.length; i++) {
                ...
                for (int j=0; j < sTempCacheList.length; j++) {
                    ...
                    //sXML+=tmp;
                    toJsonTMP = new JSONObject();
                    toJsonTMP.put("TMP",tmp);
                }
            }
        ...
        JSONObject toJson = XML.toJSONObject(sXML);
        out.println(toJson);
        out.println(toJsonTMP);
        ...
        StringReader srXML = new StringReader(toJson.toString());
...
//
            ...
            while( rs.next() )
            {
                data = new JsonObject();
                hasNone = false;
                campName = rs.getString(2);
                startDate = rs.getDate(4);
                bBacks = rs.getInt(10);
                reaching = rs.getInt(11);
                distinctReads = rs.getInt(12);
                totalReads = rs.getInt(13);
                multiReads = rs.getInt(14);
                unsubs = rs.getInt(15);
                totalLinks = rs.getInt(16);
                totalClicks = rs.getInt(17);
                totalText = rs.getInt(18);
                totalHtml = rs.getInt(19);
                distinctClicks = rs.getInt(21);
                distinctText = rs.getInt(22);
                distinctHtml = rs.getInt(23);
                oneLinkMultiClickers = rs.getInt(25);
                multiLinkClickers = rs.getInt(26);
                bBackPrc = rs.getDouble(27);
                reachingPrc = rs.getDouble(28);
                distinctReadPrc = rs.getDouble(29);
                unsubPrc = rs.getDouble(30);
                distinctClickPrc = rs.getDouble(31);
                totalTextPrc = rs.getDouble(32);
                totalHTMLPrc = rs.getDouble(33);
                distinctTextPrc =rs.getDouble(35);
                distinctHTMLPrc =rs.getDouble(36);
                sent = rs.getInt(9);
                ...
                data.put("distinctReads",distinctReads);
                data.put("totalReads",totalReads);
                data.put("multiReads",multiReads);
                data.put("unsubs",unsubs);
                data.put("totalLinks",totalLinks);
                data.put("totalClicks",totalClicks);
                data.put("totalText",totalText);
                data.put("totalHtml",totalHtml);
                data.put("distinctClicks",distinctClicks);
                data.put("distinctText",distinctText);
                data.put("distinctHtml",distinctHtml);
                data.put("oneLinkMultiClickers",oneLinkMultiClickers);
                data.put("multiLinkClickers",multiLinkClickers);
                data.put("bBackPrc", bBackPrc);
                data.put("reachingPrc",reachingPrc);
                data.put("distinctReadPrc",distinctReadPrc);
                data.put("unsubPrc",unsubPrc);
                data.put("distinctClickPrc",distinctClickPrc);
                data.put("totalTextPrc",totalTextPrc);
                data.put("totalHTMLPrc",totalHTMLPrc);
                data.put("distinctTextPrc",distinctTextPrc);
                data.put("distinctHTMLPrc",distinctHTMLPrc);
                data.put("sent",sent);
...
// <MultiClickers> 	  - Number of recipients who clicked link multiple times
            ...
            while( rs.next() )
            {
                ...
                campId = rs.getInt(1);
                hrefId = rs.getInt(2);
                linkName = rs.getString(3);
                totalClicks = rs.getInt(4);
                totalText = rs.getInt(5);
                totalHtml = rs.getInt(6);
                distinctClicks = rs.getInt(8);
                distinctText  = rs.getInt(9);
                distinctHtml = rs.getInt(10);
                multiClickers = rs.getInt(12);
                totalClickPrc = rs.getDouble(13);
                totalTextPrc = rs.getDouble(14);
                totalHtmlPrc = rs.getInt(15);
                distinctClickPrc = rs.getDouble(17);
                distinctTextPrc = rs.getDouble(18);
                disctinctHtmlPrc = rs.getDouble(19);

                data.put("campId",campId);
                data.put("hrefId",hrefId);
                ...
                data.put("distinctText",distinctText);
                data.put("distinctHtml",distinctHtml);
                data.put("multiClickers",multiClickers);
                data.put("totalClickPrc",totalClickPrc);
                data.put("totalTextPrc",totalTextPrc);
                data.put("totalHtmlPrc",totalHtmlPrc);
                data.put("distinctClickPrc",distinctClickPrc);
                data.put("distinctTextPrc",distinctTextPrc);
                data.put("disctinctHtmlPrc",disctinctHtmlPrc);
...
// <MultiSubmitters>  - Number of recipients who submitted form multiple times
            ...
            while( rs.next() )
            {
                ...
                campId = rs.getInt(1);
                firstFormId = rs.getInt(2);
                lastFormId = rs.getInt(3);
                firstFormName = rs.getString(4);
                lastFormName = rs.getString(5);
                totalViews = rs.getInt(6);
                totalSubmits = rs.getInt(7);
                distinctViews = rs.getInt(8);
                disctinctSubmits = rs.getInt(9);
                multiSubmitters =rs.getDouble(10);
                totalViewSubmitPrc = rs.getDouble(11);
                distinctViewSubmitPrc = rs.getDouble(12);

                data.put("campId",campId);
                data.put("firstFormId",firstFormId);
                data.put("lastFormId",lastFormId);
                data.put("lastFormName",lastFormName);
                data.put("totalViews",totalViews);
                data.put("totalSubmits",totalSubmits);
                data.put("distinctViews",distinctViews);
                data.put("disctinctSubmits",disctinctSubmits);
                data.put("multiSubmitters",multiSubmitters);
                data.put("totalViewSubmitPrc",totalViewSubmitPrc);
                data.put("distinctViewSubmitPrc",distinctViewSubmitPrc);
                data.put("firstFormName",firstFormName);
...
// <BBacks>           - Number of Bounce Backs
            ...
            while( rs.next() )
            {
                ...
                campId = rs.getInt(1); // !!!!!GetString
                categoryId = rs.getString(2);
                categoryName = rs.getString(3);
                bBacksS = rs.getString(4);
                bBackPrcC = rs.getString(5);

                data.put("campId",campId);
                data.put("categoryId",categoryId);
                data.put("categoryName",categoryName);
                data.put("bBacks", bBacksS);
                data.put("bBackPrc", bBackPrcC);
//                Result+="<BounceBacks>\n";
//                Result+="<CampID>"+sCampID+"</CampID>\n";
...
// <Unsubs>           - Number of Unsubscribes
            ...
            while( rs.next() )
            {
                ...
                iCampId = rs.getInt(1);
                levelId = rs.getInt(2);
                levelName = rs.getString(3);
                unsubs = rs.getInt(4);
                unsubsPrc = rs.getDouble(5);

                data.put("iCampId", iCampId);
                data.put("levelId",levelId);
                data.put("levelName",levelName);
                data.put("unsubs",unsubs);
                data.put("unsubsPrc",unsubsPrc);
                data.put("sClassAppend",sClassAppend);
...
// <UnsubsSpamPrc>     - Number of spam complaints
            ...
            while( rs.next() )
            {
                ...
                sent = rs.getInt(3);
                bbacks = rs.getInt(4);
                bbackPrc = rs.getDouble(5);
                reads = rs.getInt(6);
                readPrc = rs.getDouble(7);
                clicks =rs.getInt(8);
                clickPrc = rs.getDouble(9);
                unsubs = rs.getInt(10);
                unsubPrc = rs.getDouble(11);
                spam = rs.getInt(12);
                spamPrc = rs.getDouble(13);
...
// <Optouts>          - Number of Opt-outs
            ...
            while( rs.next() )
            {
                ...
                optOuts = rs.getInt(4);
                optOutPrc =rs.getDouble(5);

                data.put("iCampId", iCampId);
                data.put("iAttrId", iAttrId);
                data.put("attrName",attrName);
                data.put("optOuts", optOuts);
                data.put("optOutPrc", optOutPrc);
                data.put("sClassAppend",sClassAppend);
            }
            array = new JsonArray();
            array.put(data);
            ...
            if ("1".equals(sCache))
            {
                ...
                {
                    data = new JsonObject();
                    String cacheStartDate = rs.getString(1);
                    String cacheEndDate = rs.getString(2);
                    String userID = rs.getString(3);
                    String attrID = rs.getString(4);
                    String displayName = rs.getString(5);
                    String attrValue = rs.getString(6);
                    String attrValue2 = rs.getString(7);
                    String sqlName = rs.getString(8);

                    data.put("cacheStartDate",cacheStartDate);
                    data.put("cacheEndDate",cacheEndDate);
                    data.put("userID",userID);
                    data.put("attrID",attrID);
                    data.put("displayName",displayName);
                    data.put("attrValue",attrValue);
                    data.put("attrValue2",attrValue2);
                    data.put("sqlName",sqlName);
