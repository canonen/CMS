...
try	{
	...
	if (!sEnableFlag.equals("0")) {
		...
		while (rs.next()) {
			...
			data.put("attrName",rs.getString(1));
			array.put(data);
		}
		rs.close();
		...
		if (!nonEmailFinger) {
			...
			while( rs.next() ) {
				data = new JsonObject();
				String deg=rs.getString(1)+"";
				if(batchid.equals(deg))
				{  
				data.put("batchId",deg);
				data.put("batchName",rs.getString(2));
}
				else {
					data.put("batchId",rs.getString(1));
					data.put("batchName",rs.getString(2));
}
			array.put(data);
			}
rs.close();
				...
				{
					data.put("sEmailType",sEmailType[i]);
					data.put("nEmailType",iEmailTypeId[i]);
					array.put(data);
				}
