...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		<td class="SpecialTab<%=("Standard".equals(sCampTypeLabel))?"On":"Off"%>"
			width="<%= campWidth1 %>" valign="center" nowrap align="middle"
			onclick="FT.type_id.value = 2; FT.auto_queue_daily_flag.value = 0; GO(0);"
			<%= (!showCamp1)?" style=\"display:none;\"":"" %>>
			Standard
		</td>
		<td class="SpecialTab<%=("Send To Friend".equals(sCampTypeLabel))?"On":"Off"%>"
			width="<%= campWidth2 %>" valign="center" nowrap align="middle"
			onclick="FT.type_id.value = 3; GO(0);"
			<%= (!showCamp2)?" style=\"display:none;\"":"" %>>
			Send To Friend
		</td>
		<td class="SpecialTab<%=("Automated".equals(sCampTypeLabel))?"On":"Off"%>"
			width="<%= campWidth3 %>" valign="center" nowrap align="middle"
			onclick="FT.type_id.value = '4'; GO(0);"
			<%= (!showCamp3)?" style=\"display:none;\"":"" %>>
			Automated
		</td>
		<td class="SpecialTab<%=("Web/DM/Call".equals(sCampTypeLabel))?"On":"Off"%>"
			width="<%= campWidth4 %>" valign="center" nowrap align="middle"
			onclick="FT.type_id.value = 5; GO(0);"
			<%= (!showCamp4)?" style=\"display:none;\"":"" %>>
			Web/DM/Call
		</td>
	</tr>
</table>
...
if (can.bWrite)
{
	...
	{
		%>
		<td nowrap align="left" valign="middle"><a class="newbutton" href="javascript:openModal('camp_new.jsp?type_id=' + FT.type_id.value, document, '500', '400');">New Campaign</a>&nbsp;&nbsp;&nbsp;</td>
		<%
	}
	else
	{
		...
		{
			%>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="camp_edit.jsp?type_id=4&media_type_id=1&<%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>">New Triggered Campaign</a>&nbsp;&nbsp;&nbsp;</td>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="camp_edit.jsp?type_id=2&media_type_id=1&&auto_queue_daily_flag=1<%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>">New Check Daily Campaign</a>&nbsp;&nbsp;&nbsp;</td>
	        <% if (canPrint) { %>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="camp_edit.jsp?type_id=4&media_type_id=2&<%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>">New Triggered Print Campaign</a>&nbsp;&nbsp;&nbsp;</td>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="camp_edit.jsp?type_id=2&media_type_id=2&auto_queue_daily_flag=1<%=(sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:""%>">New Check Daily Print Campaign</a>&nbsp;&nbsp;&nbsp;</td>
	        <% } %>
			<%
		}
		else
		{
			%>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="<%=sEditHref%><%="&media_type_id=1"%>">New <%=sCampTypeLabel%> Campaign</a>&nbsp;&nbsp;&nbsp;</td>
			<%
			if (TYPE_ID.equals("2")) {
			%>
	        <% if (canPrint) { %>
			<td nowrap align="left" valign="middle"><a class="newbutton" href="<%=sEditHref%><%="&media_type_id=2"%>">New <%=sCampTypeLabel%> Print Campaign</a>&nbsp;&nbsp;&nbsp;</td>
	        <% } %>
			<%
			}
		}
...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		...
		sActiveCampId = cDAO.getActiveCamp(s_origin_camp_id,null);
			%>
				<tr>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_status_name %></td>
					<td class="listItem_Data<%= sClassAppend %>"><a href="camp_edit.jsp?camp_id=<%= s_origin_camp_id %>&type_id=<%= TYPE_ID %>&media_type_id=<%= s_media_type_id %><%= (sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:"" %>" target"=_self"><%= s_camp_name %></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_type_id_name %></td>
					<% if (canPrint) { %><td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_media_type_id_name %></td><% } %>
					<td class="listItem_Title<%= sClassAppend %>" nowrap><%= s_start_date %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_qty_queued %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_qty_sent %></td>
		<%
		if (canApprove.bExecute)
		{
			...
			%>
					<td class="listItem_Data<%= sClassAppend %>" align="center" nowrap style="padding:5px;">
			<%
...
<table cellspacing="0" cellpadding="0" width="100%" border="0">
	...
	while( rs.next() )
	{
		...
		%>
				<tr>
					<td class="listItem_Data<%= sClassAppend %>"><a href="camp_edit.jsp?camp_id=<%= s_origin_camp_id %>&type_id=<%= TYPE_ID %>&media_type_id=<%= s_media_type_id %><%= (sSelectedCategoryId!=null)?"&category_id="+sSelectedCategoryId:"" %>"><%= s_camp_name %></a></td>
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_type_id_name %></td>
					<% if (canPrint) { %><td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_media_type_id_name %></td><% } %>
					<td class="listItem_Title<%= sClassAppend %>" nowrap><%= s_modified_date %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_filter_name %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_cont_name %></td>
				</tr>
<%
	}
	rs.close();
...
%>
					<tr>
						<td class="listItem_Data" colspan="6" align="left" valign="middle">There are currently no draft campaigns being worked on.</td>
					</tr>
<%	} %>
			</table>
		</td>
...
%>		
				<tr>
					...
					<td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_type_id_name %></td>
					<% if (canPrint) { %><td class="listItem_Data<%= sClassAppend %>" nowrap><%= s_media_type_id_name %></td><% } %>
					<td class="listItem_Title<%= sClassAppend %>" nowrap><%= (useEnd)?s_end_date:s_finish_date %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_filter_name %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_cont_name %></td>
					<td class="listItem_Data<%= sClassAppend %>"><%= s_status_name %></td>
...
%>
				<tr>
					<td class="listItem_Data" colspan="<%= (canRept.bRead)?"7":"6" %>" align="left" valign="middle">There are no completed campaigns.</td>
				</tr>
<%	} %>
			</TABLE>
		</td>
