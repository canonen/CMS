...
{
	%>
		<td class="EditTabOn" id="tab_<%=nTabId%>_header_<%=(++nTabHeaderId)%>" width="150" onclick="switchSteps('tab_<%=nTabId%>', 'tab_<%=nTabId%>_header_<%=nTabHeaderId%>', 'tab_<%=nTabId%>_page_<%=nTabHeaderId%>');" valign="middle" nowrap align="center">Campaign Settings</td>
	<%
	if (step2_tab2_show)
	{
		%>
		<td class="EditTabOff" id="tab_<%=nTabId%>_header_<%=(++nTabHeaderId)%>" width="150" onclick="switchSteps('tab_<%=nTabId%>', 'tab_<%=nTabId%>_header_<%=nTabHeaderId%>', 'tab_<%=nTabId%>_page_<%=nTabHeaderId%>');" valign="middle" nowrap align="center">Advanced Settings</td>
		<%
	}
	if (step2_tab3_show)
	{
		%>
		<td class="EditTabOff" id="tab_<%=nTabId%>_header_<%=(++nTabHeaderId)%>" width="150" onclick="switchSteps('tab_<%=nTabId%>', 'tab_<%=nTabId%>_header_<%=nTabHeaderId%>', 'tab_<%=nTabId%>_page_<%=nTabHeaderId%>');" valign="middle" nowrap align="center">Restrictions</td>
		<%
	}
	%>
		<td class="EmptyTab" valign="middle" nowrap align="center" width="<%= step2_tab_width %>">&nbsp;</td>
	<%
}
else
{
