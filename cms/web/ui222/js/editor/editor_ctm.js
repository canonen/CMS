...
/* ---------------------------------------------------------------------- *\
  Function    : editor_hyattConfig
  Description : Hyatt's configuration settings for wysiwyg editor
\* ---------------------------------------------------------------------- */

function editor_hyattConfig(objname) {
...
/* ---------------------------------------------------------------------- *\
  Function    : editor_simpleConfig
  Description : simple configuration settings for wysiwyg editor
\* ---------------------------------------------------------------------- */

function editor_simpleConfig(objname) {
...
this.toolbar = [['bold','italic','underline','separator']];
...
function editor_generate(objname,userConfig) {
  ...
  if (userConfig && userConfig == "simple") { 
	config = new editor_simpleConfig(objname);
  }
  else if (userConfig && userConfig == "hyatt") { 
	config = new editor_hyattConfig(objname);
  }
  ...
  // catch keypress events
    if (editEvent && editEvent.keyCode) {
      ...
      if (ord == 18) { return; }  // ignore alt key by itself
       ...
       if (config.suppressCR == 1 && ord == 13 && editEvent.type == 'keypress') {
...
function editor_updateOutput(objname) {
  ...
  contents = contents.replace(/\&amp\;/g, '&'); // undo the & to &amp; conversion automatically done by internet explorer
