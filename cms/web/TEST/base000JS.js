(async function() {
//customer0 dolduruyor

//All customers
    var rcpList = [1,2,3,6,7,8,9,10,11,12];
...
`); //Bu kısıma sql yazılıyor Go vs kaldırılacak orijinal sql olacak.
    ...
    for(var no of rcpList) {
        for(var sqlString of sql) {
            await fetch('http://rcp'+no+'.revotas.com/rrcp/imc/execCode/exec_code.jsp?cust_id=0&type='+type, {
                mode: 'no-cors',
                method: 'POST',
                headers: {
