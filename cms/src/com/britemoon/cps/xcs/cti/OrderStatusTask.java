...
public class OrderStatusTask extends BriteTask
{
	private String m_sOrderID = null;
	private static Logger logger = Logger.getLogger(OrderStatusTask.class.getName());

	public OrderStatusTask(String sOrderID) throws Exception
	{
		init(sOrderID);
	}
	...
	public void startStatic(String sOrderID) throws Exception
	{
		...
		try
		{
			OrderSetupUtil.setOrderStatus(sOrderID);
		...
		catch(Exception ex) 
		{
			...
			String sSql = "UPDATE cxcs_order"
				+ " SET status_id = " + OrderStatus.ERROR_EXECUTING + ", status_date = getdate(),"
			...
			osc.sendBriteStatus(sOrderID, OrderStatus.ERROR_EXECUTING);
