...
public class FtpImportTimer extends BriteTimer
{
	private static Logger logger = Logger.getLogger(FtpImportTimer.class.getName());
	public FtpImportTimer(String sTimerNameSuffix)
	{
	...
	public Vector buildTaskList()
	{
		...
		try
		{
			...
			conn = cp.getConnection("FtpImportTimer");
			...
			try
			{
				...
				String sTaskId = null;
				FtpImportTask fit = null;				
				while (rs.next())
				{
					sTaskId = rs.getString(1);
					fit = new FtpImportTask(sTaskId);
					vTaskList.add(fit);
				}
				rs.close();
			}
