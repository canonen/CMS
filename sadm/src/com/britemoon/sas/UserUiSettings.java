...
public class UserUiSettings extends BriteObject
{
	...
	private static Logger logger = Logger.getLogger(UserUiSettings.class.getName());
	// === Some crap from cps to prevent overwriting data on cps while synchronization ===
	...
	public void getPropsFromResultSetRow(ResultSet rs) throws Exception
	{
		...
		s_ui_type_id = rs.getString(2);
		s_recip_view_count = rs.getString(3);
		s_default_page_size = rs.getString(4);
	}

	// === DB Method save()===

	public String m_sSaveSql =
		" EXECUTE usp_scps_user_ui_settings_save" +
		"	@user_id=?," +
		"	@ui_type_id=?," +
		"	@recip_view_count=?," +
		"	@default_page_size=?";
	...
	public int saveProps(PreparedStatement pstmt) throws Exception
	{
		...
		pstmt.setString(1, s_user_id);
		pstmt.setString(2, s_ui_type_id);
		pstmt.setString(3, s_recip_view_count);
		pstmt.setString(4, s_default_page_size);
	...
	public String m_sDeleteSql =
		" DELETE FROM scps_user_ui_settings" +
		" WHERE" +
		"	(user_id=?)";
	...
	public void appendPropsToXml(Element e)
	{
		...
		if( s_recip_view_count == null ) XmlUtil.appendTextChild(e, "recip_view_count", "500");
		else XmlUtil.appendTextChild(e, "recip_view_count", s_recip_view_count);
		
		if( s_default_page_size == null ) XmlUtil.appendTextChild(e, "default_page_size", "25");
		else XmlUtil.appendTextChild(e, "default_page_size", s_default_page_size);
		
		if( s_cust_id != null ) XmlUtil.appendTextChild(e, "cust_id", s_cust_id);
		
	 	// -1 - flag recognizable on cps to preserve existing category		
		if( s_category_id == null ) XmlUtil.appendTextChild(e, "category_id", "-1");
		else XmlUtil.appendTextChild(e, "category_id", s_category_id);
	}
