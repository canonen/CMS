...
// Default the user's pv_login and pv_password to the registry values if the user values are null.
// Get all pv features in CustFeature and if any exist, then allow the pv login and password to default from the registry,
// if the pv_login and passwords are null.

if  ((CustFeature.exists(user.s_cust_id, Feature.PV_CONTENT_SCORER)) ||
     (CustFeature.exists(user.s_cust_id, Feature.PV_DELIVERY_TRACKER)) ||
     (CustFeature.exists(user.s_cust_id, Feature.PV_DESIGN_OPTIMIZER)) ||
     (CustFeature.exists(user.s_cust_id, Feature.PV_LOGIN))) {

    String defaultPVLoginId = Registry.getKey("default_pv_login_id");
    String defaultPVLoginPassword = Registry.getKey("default_pv_password");
    if ((defaultPVLoginId != null) || (defaultPVLoginPassword != null)) {
        // if customer features includes pv_login, then allow the user's pv login to default to the registry
        if (user.s_pv_login == null) {
            user.s_pv_login = defaultPVLoginId;
        }
        if (user.s_pv_password == null) {
            user.s_pv_password = defaultPVLoginPassword;
        }
    }
}
%>
