	...
	<tr>
		<td>
			<table id="Tabs_Table1" cellspacing="0" cellpadding="0" border="0" class="layout" style="width:350;">
				...
				<col>
				<tr height="20">
					<td class="EditTabOn" id="tab1_Step1" valign="center" nowrap align="middle">Features</td>
				...
				<tr>
					<td class="fillTab" valign="top" align="center" colspan="2">
						<form name="FT" method="POST" action="cust_features_save.jsp">
...
<%
	try
	{
 		...
 		sSql  = " SELECT p.feature_id, p.feature_name, cp.cust_id";
		sSql += " FROM sadm_feature p";
		sSql += 	" LEFT OUTER JOIN sadm_cust_feature cp";
		sSql += 		" ON ( p.feature_id = cp.feature_id )";
		sSql += 		" AND ( cp.cust_id = ?)";
		sSql += " ORDER BY p.feature_id";
		...
		while (rs.next())
		{
			sFeatureId = rs.getString(1);
			sFeatureName = rs.getString(2);
			sId = rs.getString(3);
%>
	<tr>
		<td class="listItem_Data"><%= sFeatureName %></td>
		<td class="listItem_Data" align="center">
			<input type="checkbox" name="feature_id" value=<%=sFeatureId%> <%=(sId!=null)?"checked":""%>>
		</td>
	</tr>
<%
		}
