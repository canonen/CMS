...
try {
	...
	conn = cp.getConnection("mod_version_service_save.jsp");
	stmt = conn.createStatement();

	sSQL = "SELECT count(*) FROM sadm_mod_version_service"
		+ " WHERE mod_id = "+sModID
	...
	if (nCount < 1) {
		sSQL = "INSERT sadm_mod_version_service (protocol, port, path, mod_id, version, service_type_id)"
			+ " VALUES (?,?,?,?,?,?)";
		prepStmt = conn.prepareStatement(sSQL);
	} else {
		sSQL = "UPDATE sadm_mod_version_service SET protocol=?, port=?, path=?"
			+ " WHERE mod_id=? AND version=? AND service_type_id=?";
		prepStmt = conn.prepareStatement(sSQL);
	}
	...
	prepStmt.setString(4, sModID);
	prepStmt.setString(5, sVersion);
	prepStmt.setString(6, sServTypeID);
