...
<head>
	...
	function loadParentOptions()
	{
		...
		else if (selType == "102")
		{
			...
			try
			{
				sSql = "select help_doc_id, internal_heading from shlp_help_doc where type_id = 101 order by help_order";
				pstmt = conn.prepareStatement(sSql);
				rs = pstmt.executeQuery();
				while (rs.next())
				{
		...
		else if (selType == "103")
		{
			...
			try
			{
				sSql = "select c.help_doc_id, v.internal_heading + ': ' + c.internal_heading as 'InternalHeading'" +
						" from shlp_help_doc c with(nolock)" +
						" inner join shlp_help_doc v with(nolock) on c.parent_help_doc_id = v.help_doc_id" +
						" where c.type_id = 102" +
